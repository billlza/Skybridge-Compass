...
class AppleReleaseInteropOffererAppInstrumentationTest {
    ...
    fun hostsCodeForAppleResponderUsingAppProcess() = runBlocking {
        ...
        try {
            ...
            println(
                "SB-ANDROID-APP-OFFER binding deviceId=${localBinding.deviceId} fingerprint=${localBinding.protocolPublicKeyFingerprint}"
            )
            manager.setPqcEnabled(pqcEnabled)
            val code = manager.generateConnectionCode()
            println("SB-ANDROID-APP-OFFER code=$code")
                    ...
                    when (val state = manager.state.value) {
                        ...
                        is SkyBridgeWebRtcConnectionManager.State.Connected -> {
                            ...
                            if (hasSessionKeys && hasBootstrapKem && hasExpectedSuite) {
                                println(
                                    "SB-ANDROID-APP-OFFER success code=${state.code} bootstrapKem=$hasBootstrapKem suite=${manager.negotiatedSuiteName() ?: "-"}"
                                )
            ...
            assertTrue("Expected app-layer session keys after Apple responder joined", manager.hasSessionKeys())
            if (pqcEnabled) {
                assertTrue(
                ...
                assertTrue(
                    "Expected PQC rekey to complete after Apple responder joined",
