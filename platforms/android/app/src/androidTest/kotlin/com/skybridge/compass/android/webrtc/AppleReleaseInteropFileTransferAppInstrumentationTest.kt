...
class AppleReleaseInteropFileTransferAppInstrumentationTest {

    @Test
    fun joinsAppleSmokeHostAndSendsFileUsingAppProcess() = runBlocking {
        ...
        try {
            ...
            if (!peerMetadataReady) {
                error(
                    "Timed out waiting for authenticated peer metadata before file transfer; " +
            ...
            val payload = buildString {
                appendLine("skybridge-android-apple-file-transfer-smoke")
            ...
            controller.sendBytesAsFile(
                ...
                fileName = "android-apple-smoke.txt",
                mimeType = "text/plain",
                bytes = payload,
                chunkSize = 16 * 1024
            )
            println(
                "SB-ANDROID-APP-SMOKE fileTransferSent " +
            ...
            val completionAck: CrossNetworkFileTransferMessage =
                ...
                while (true) {
                    if (transfer.peerError.isCompleted) {
                        ...
                        error(
                            "macOS file-transfer error: ${peerError.message ?: "unspecified"}; " +
                    ...
                    when (val state = manager.state.value) {
                        is SkyBridgeWebRtcConnectionManager.State.Failed ->
                            error(
                                "WebRTC failed while waiting for completeAck: ${state.message}; " +
                                    "outboundOps=${transfer.outboundOpsSnapshot()} " +
                                    "inboundOps=${transfer.inboundOpsSnapshot()} " +
                                    "progress=${controller.progress.value} " +
                                    "signaling=${manager.signalingStatus.value.lastEvent}"
                ...
                "Timed out waiting for macOS completeAck after ${fileTransferTimeoutSeconds}s; " +
                    "outboundOps=${transfer.outboundOpsSnapshot()} " +
                    "inboundOps=${transfer.inboundOpsSnapshot()} " +
                    "progress=${controller.progress.value} " +
                    "managerState=${manager.state.value} " +
            ...
            assertEquals(
                "Expected macOS completeAck receivedBytes to match payload",
                payload.size.toLong(),
                completionAck.receivedBytes
            )
            assertArrayEquals(
                "Expected macOS completeAck fileSha256 to match payload",
                payloadSha256,
            ...
            assertTrue(
                "Expected macOS ACK sequence to include metadataAck/chunkAck/completeAck; " +
            ...
            val completionRoute = routeExpectation.requireAdmittedAtCompletion(
                manager = manager,
                secureOwner = transfer.exactFileTransferOwner()
            )
            println(
                "SB-ANDROID-APP-SMOKE success code=<redacted> " +
                    "bootstrapKem=$observedBootstrapKem " +
                    "bootstrapQPeriapt=${if (suiteExpectation.expectsQPeriapt) observedBootstrapKem else manager.hasBootstrappedPeerQPeriaptForCurrentPeer()} " +
                    ...
                    "payloadSha256=${payloadSha256.toHex()} " +
                    "outboundOps=${transfer.outboundOpsSnapshot().joinToString(",")} " +
                    "inboundAcks=${transfer.inboundOpsSnapshot().joinToString(",")} " +
