...
def _unique_json_object(pairs: list[tuple[str, object]]) -> dict[str, object]:
    ...
    for key, item in pairs:
        if key in value:
            raise OwnershipError(f"JSON object contains duplicate key: {key}")
        value[key] = item
    return value


def _reject_json_constant(value: str) -> object:
    raise OwnershipError(f"JSON contains non-finite number: {value}")
...
def _normalized_file_url_path(value: object, field: str) -> str:
    ...
    if re.search(r"%(?![0-9A-Fa-f]{2})", parsed.path):
        raise OwnershipError(f"{field} contains invalid URL encoding")
    try:
    ...
    if urllib.parse.quote(decoded, safe="/-._~") != parsed.path:
        raise OwnershipError(f"{field} path is not canonically percent-encoded")
