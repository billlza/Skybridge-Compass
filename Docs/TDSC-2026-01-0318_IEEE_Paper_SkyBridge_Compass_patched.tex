...
\newcommand{\artifacttag}{tdsc-2026-01-0318-ios-sim-fix-20260211}
\newcommand{\artifactsha}{b16fe9386ff0}
\newcommand{\artifactshafull}{b16fe9386ff047d17ba9a518b2c331f64493971e}
\newcommand{\artifactzipurl}{\artifacturl/archive/\artifactshafull.zip}
\newcommand{\artifactzipsha}{460813da\allowbreak\hspace{0pt}9ade5b79\allowbreak\hspace{0pt}b333c94a\allowbreak\hspace{0pt}1b76e14e\allowbreak\hspace{0pt}6b522f63\allowbreak\hspace{0pt}0be1a6f0\allowbreak\hspace{0pt}decb9845\allowbreak\hspace{0pt}def3f258}
\newcommand{\artifacttarurl}{\artifacturl/archive/\artifactshafull.tar.gz}
\newcommand{\artifacttarsha}{443b8ac9\allowbreak\hspace{0pt}0ffe42f7\allowbreak\hspace{0pt}57d65f1d\allowbreak\hspace{0pt}79cd3d18\allowbreak\hspace{0pt}3b7d7ce5\allowbreak\hspace{0pt}02dd8bcd\allowbreak\hspace{0pt}31cdb016\allowbreak\hspace{0pt}1016f380}
\newcommand{\artifactdate}{2026-01-23}
\newcommand{\artifactdateXWing}{2026-01-16}
...
1.89 ms (p95 2.18 ms) for classic, 3.47 ms (p95 4.18 ms) for liboqs PQC,
and 5.20 ms (p95 6.16 ms) for CryptoKit PQC over N=1000 iterations.
The handshake wire size (MessageA + MessageB + 2$\times$Finished) grows
from 687 B (classic) to 12,002 B (PQC). Beyond
...
\item \textbf{Downgrade resistance (policy-defined):} strict policy is
  ...
  and requires PQC rekey before business traffic. The default policy
...
\item
  ...
  including strict-policy separation into Compliance vs
  Bootstrap-Assisted realizations.
...
providers, treating Apple's CryptoKit PQC \cite{ref1} as a deployment
...
\textbf{Implementation alignment (GitHub HEAD, 2026-02-08).} In the
...
\texttt{strictPQC} corresponds to requiring PQC with either (i)
Compliance Mode (no classic establishment) or (ii)
Bootstrap-Assisted Mode (control-only bootstrap plus mandatory PQC
rekey).
...
policy gate before any default-mode fallback attempt; strict compliance
forbids classic establishment edges. For strict recovery, bootstrap is
trigger-limited, emits explicit \texttt{handshake\_fallback} events, and
enforces control-only messaging until PQC rekey succeeds. This is
validated by policy downgrade benchmarks
...
Default-policy classic fallback emits \texttt{cryptoDowngrade} with full
context (reason, deviceId, cooldown), enabling audit and anomaly
detection. Strict Bootstrap-Assisted recovery emits
\texttt{handshake\_fallback} events with mode
\texttt{bootstrap\_assisted}
and trigger/result fields, so reviewers can distinguish ``strict
compliance fail-closed'' from ``strict recovery with mandatory PQC
rekey'' traces.
...
policy; strictPQC Compliance Mode denies classic fallback edges
...
configurations. Classic handshakes remain near 2~ms (2.3~ms at p99); CryptoKit
PQC stays under 6.7~ms at p99 with variance dominated by ML-DSA signature
...
(submitted as separate file \texttt{TDSC-2026-01-0318\_supplementary.pdf}).
...
The $\approx$17$\times$ size increase from Classic (687~B) to PQC (12.0~kB) is dominated by ML-DSA signatures (3309~B) and ML-KEM ciphertexts (1088~B). Fig.~\ref{fig:message-size-breakdown} shows the per-field breakdown.
...
identity fields, framing overhead) for MessageA/MessageB across Classic and PQC configurations. Counts reflect MessageA/B payloads only; the \emph{payload-only} handshake size adds two 38~B Finished frames (accounted for in Table~\ref{tab:perf-summary}), and detailed field sizes are reported in Supplementary Table~\ref{tab:supp-message-sizes}. Generated from \texttt{Artifacts/message\_sizes\_\artifactdate.csv}.}
\label{fig:message-size-breakdown}
\end{figure}

Note: X-Wing hybrid KEM adds 32~B to MessageA's keyshare (1120~B vs 1088~B for ML-KEM-768). Using the X-Wing snapshot (\texttt{\artifactdateXWing}), the measured total handshake size is 12,195~B (\texttt{MessageA.XWing}/\texttt{MessageB.XWing}); this is +32~B versus PQC in the same snapshot, while the additional +161~B vs the 12,002~B baseline reflects earlier framing overhead. Detailed field breakdowns are in Supplementary Table~\ref{tab:supp-message-sizes}.
...
% Tuned to avoid an almost-empty trailing references page.
\IEEEtriggeratref{39}
