...
(p50/p95) are reported in Supplementary Table~S6 and are reproducible via
loopback pcap capture
(\texttt{Scripts/run_baseline_bench.sh}).
SkyBridge-CryptoKit loopback baselines are included in Supplementary Table~S6.
...
sketches tied to protocol mechanics. Formal verification using model checking or symbolic analysis [22] is planned for future work:
...
\noindent\textit{TABLE VIII. Performance Summary. All benchmarks on Apple Silicon (M1/M3), macOS 26.x, N=1000 iterations after 10 warmup runs. Wire Size counts payload-only handshake bytes (MessageA + MessageB + 2×Finished); loopback wire sizes including transport overhead are reported separately in Table~\reftab:baseline-comparison and Supplementary Table~S4. Data-plane AEAD is fixed to AES-256-GCM in v1, so throughput is independent of the negotiated handshake suite; throughput measured post-handshake on 1~MiB payloads.}\par
