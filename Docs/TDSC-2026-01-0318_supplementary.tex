...
\item X-Wing (\texttt{\artifactdateXWing} snapshot): $6609 + 5510 + 2 \times 38 = 12{,}195$~B
\end{itemize}
...
breakdown is:
\begin{itemize}\tightlist
\item MessageA.XWing: $3309$ (signature) + $1120$ (keyshare) + $1956$
  (identity) + $224$ (overhead) $= 6609$~B
\item MessageB.XWing: $3309$ (signature) + $0$ (keyshare) + $1956$
  (identity) + $245$ (overhead) $= 5510$~B
\item Finished: $38$~B
\end{itemize}
Total handshake size is \textbf{12,195~B}. Within the same snapshot, the
PQC baseline is 12,163~B, so X-Wing adds +32~B. The +193~B delta vs the
12,002~B baseline in the main paper reflects the +32~B keyshare and a
snapshot delta of +161~B from earlier framing overhead.
