...
\item Table~\ref{tab:supp-boundary-stress-empirical} (Boundary-stress empirical invariants): machine-checkable rates for input perturbation, concurrency/race, adversarial-network timeout suppression, and evidence-schema integrity from \texttt{boundary\_stress\_<date>.csv}.
...
BaselineBenchRunner measures \emph{time-to-ready}
(\texttt{NWConnection.ready}) for fresh loopback connections. Each sample
creates a new connection and records elapsed time from connect start to
ready. We report N=1000 after 10 warmup iterations. 0-RTT/session
resumption is not explicitly enabled and results reflect
\texttt{Network.framework} defaults. QUIC uses ALPN \texttt{sbq} with a
single bidirectional stream and no application data
(\texttt{BASELINE\_KICKOFF\_BYTES=0}). Loopback wire-size values are
reproducible from pcap capture via
...
\item \texttt{Artifacts/\allowbreak boundary\_\allowbreak stress\_2026-02-24.csv} (Table~\ref{tab:supp-boundary-stress-empirical})
\item \texttt{Artifacts/\allowbreak baseline\_\allowbreak summary\_*.csv} (Table~\ref{tab:supp-loopback-wire})
\item \texttt{Artifacts/\allowbreak soa\_\allowbreak interop\_2026-02-22.csv} (Table~\ref{tab:soa-interop})
\end{itemize}
...
We separate source-quality, paper-integrity, and release-readiness gates
to avoid cross-domain false failures and to preserve auditability.
New zero-warning and layered-gate reports are
\textbf{release-readiness auxiliary artifacts}; they are additive and do
not change the core artifact schema (\texttt{handshake\_bench\_*},
\texttt{handshake\_rtt\_*}, \texttt{message\_sizes\_*}, and
claims/macro datasets).

\subsection*{Frozen Invariants for the Upgrade Track}
\begin{itemize}\tightlist
\item wire format unchanged (MessageA/MessageB/Finished)
\item suite IDs unchanged (including wireId assignments)
\item canonical message vectors unchanged
\item artifact date fixed at \texttt{2026-01-23}
\item primary experimental numbers unchanged
\item threat model unchanged
\item policy semantics unchanged (strict/prefer/classic)
\item fallback deny semantics unchanged
\item main paper claims unchanged
\end{itemize}

\subsection*{Layered Gate Definitions and Failure Attribution}
\begin{itemize}\tightlist
\item \textbf{source-quality gate:} SwiftPM build/test, iOS build/test lane,
  and zero-warning enforcement only for source/toolchain outcomes.
\item \textbf{paper-integrity gate:} isolated paper compilation, claim/numeric
  consistency checks, and PDF quality checks (Type3/text extraction/metadata)
  without writing back tracked PDF outputs.
\item \textbf{release-readiness gate:} aggregation-only layer over the two
  domain gates; no additional cross-domain checks are injected.
\item \textbf{failure attribution contract:} reports always expose
  \texttt{failed\_domain}, \texttt{failed\_check\_id}, and
  \texttt{owner\_hint} (code/paper/script/environment) for auditability.
\end{itemize}
...
layer can then be replaced with direct CryptoKit PQ-HPKE calls. The
...
\IfFileExists{supp_tables/s25_boundary_stress_empirical.tex}{\input{supp_tables/s25_boundary_stress_empirical}}{\noindent\textit{(Boundary-stress empirical table not present: run \texttt{SKYBRIDGE\_RUN\_BOUNDARY\_STRESS=1 swift test --filter SkyBridgeCoreTests.BoundaryStressBenchTests}, then \texttt{python3 Scripts/aggregate\_boundary\_stress.py --artifact-date <date>}.)}\par}
...
\textbf{SOA-specific regression extension (this revision):}
we additionally run \texttt{SOAInteroperabilityBenchTests}
(\texttt{N=100} per row) to verify deterministic concurrent-open
arbitration, authenticated supersede binding checks, established-guard
release semantics, supersede rate limiting, and legacy/new MessageA
compatibility with raw TLV passthrough
(\path{Artifacts/soa_interop_2026-02-22.csv}).
...
`ConcurrentOpenSingleFlight` & \texttt{SupersedeAccepted} + \texttt{LocalWinnerRetained} & \path{Sources/SkyBridgeCore/P2P/PeerSessionArbiter.swift} + \path{Sources/SkyBridgeCore/P2P/HandshakeDriver.swift} (byte-ordered tuple arbitration) & \path{Artifacts/soa_interop_2026-02-22.csv}; Table~\ref{tab:soa-interop} \\
`AuthenticatedSupersedeBinding` & \texttt{BindingCheckPassed} & \path{Sources/SkyBridgeCore/P2P/HandshakeDriver.swift} + \path{Sources/SkyBridgeCore/P2P/P2PDiscoveryService.swift} (signature-first + canonical pair binding) & \path{Artifacts/soa_interop_2026-02-22.csv} (forged-target row); Table~\ref{tab:soa-interop} \\
\bottomrule
\end{tabular}
\end{table*}
