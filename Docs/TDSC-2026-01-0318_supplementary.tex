...
\setlength{\@dblfpsep}{6pt plus 2pt minus 2pt}
...
\centering
\scriptsize
\caption{Dedicated EC2 direct-path validation (external Linux, no tunnel, \texttt{ARTIFACT\_DATE=2026-01-23}). Endpoint: \texttt{54.92.79.99:44444}; harness: \texttt{Scripts/run\_real\_network\_e2e\_linux.py}; \texttt{n=50} per payload.}
...
\item \texttt{suite\allowbreak Negotiation\allowbreak Failed} (local pre-send branch only)
\end{itemize}

\textbf{Blocked:}
\begin{itemize}\tightlist
\item \texttt{timeout}
\item \texttt{suite\allowbreak Negotiation\allowbreak Failed} after remote input / after-send
...
baseline (12,002~B), the +134~B delta reflects +128~B X-Wing keyshare
expansion and +6~B framing overhead in this snapshot.
...
For \texttt{Artifacts/system\_impact\_2026-01-23.csv}, we performed one
integrity cleanup pass and removed one duplicated embedded header row
before table generation. Final file summary:
\begin{itemize}\tightlist
...
\item No fully duplicated data rows remain
\end{itemize}
...
baselines (classic 687~B, PQC 12{,}002~B; transport uses a fixed 4-byte
length prefix)
\end{itemize}
