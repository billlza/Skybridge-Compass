...
This paper presents an auditable cryptographic migration contract,
instantiated in SkyBridge Compass, that explicitly separates (i)
negotiated cryptographic suites, (ii) provider backends, and (iii) a
policy layer with explicit fallback semantics and a minimal evidence set
for replaying downgrade decisions. Under Default Mode, classic
establishment is never triggered by timeout or network-derived failures;
any allowed classic fallback is local-only (pre-send), rate-limited, and
emits replayable downgrade evidence. We use an actor-isolated handshake
state machine with deterministic outcomes and structured security-event
emission, providing protocol-level audit signals that make downgrade
decisions and failure semantics explicit and replayable. Signals are
linked to transcript-derived identifiers for in-session tamper-evident
ordering under a non-compromised runtime assumption; long-term retention
and remote attestation of logs are out of scope. We
evaluate a reproducible \emph{core gate} on macOS 26.x with three
configurations: classic (X25519 + Ed25519), PQC via liboqs (ML-KEM-768 +
ML-DSA-65), and liboqs PQC v2 FS (ML-KEM-768-FS + ML-DSA-65). Apple
CryptoKit PQC/X-Wing are reported as a separate \emph{contrast} dataset
and do not participate in hard pass/fail gates. To avoid
cross-domain confounding, we evaluate source-quality,
paper-integrity, and release-readiness gates independently with
traceable pass/fail evidence in each domain. To prevent
upgrade-time moving-goalpost effects, this track freezes wire formats,
suite/wire IDs, canonical vectors, the artifact date
(\texttt{2026-01-23}), threat model, and strict/prefer/classic policy
semantics (including fallback-deny behavior). Cross-platform
uplift is validated with a decision-parity matrix that fixes policy
mode, peer visibility, local provider availability, compile-time flags,
trust-store state, and bootstrap/rekey state, then requires equivalent
suite choice, fallback allow/deny, event semantics, and user-facing
error categories across macOS and iOS. We do not claim
...
\claimLiboqsVTwoMeanMs~ms (p95 \claimLiboqsVTwoPNineFiveMs~ms) for liboqs PQC v2 FS
under the core benchmark controls (N=1000 per configuration). Apple
contrast runs are collected separately (N=5000) and reported in
the supplementary Apple/X-Wing contrast report.
The handshake payload size (MessageA + MessageB + 2$\times$Finished) grows
from \claimClassicPayloadBytes~B (classic) to \claimLiboqsPayloadBytes~B
(liboqs PQC) and \claimLiboqsVTwoPayloadBytes~B (liboqs PQC v2 FS) in the
core gate. Apple PQC/X-Wing payloads are reported as contrast-only data
in the supplementary Apple/X-Wing contrast report. Beyond
...
Representative minimal event typings are summarized in
Supplementary Table~\ref{tab:supp-security-event-types} (S37).
...
\begin{enumerate}
\item
  \textbf{C1: Formalized auditable migration contract.}
  We formalize an auditable migration contract spanning four
  mechanism-level requirements (suite/provider migration, policy
...
\item
  ...
  v1 remains default for bridge compatibility with provisioned KEM
  identity keys; v2 is an opt-in upgrade (not full PFS).
  \textit{Evidence:} Fig.~\ref{fig:handshake-sequence};
  Supplementary Table~\ref{tab:supp-v2-v1-compare};
  \texttt{HandshakeV2PFSTests};
  \texttt{ProtocolSignatureRegressionTests}.
\item
  \textbf{C3: Reproducible implementation and verification workflow.}
  We ship an actor-isolated implementation that emits protocol-level
  audit signals and maintains transcript-anchored, in-session
  tamper-evident ordering under a non-compromised runtime assumption;
  long-term retention and remote attestation depend on the operating
  environment and are out of scope. We
...
\item
  \textbf{C4: Policy semantics for strict/default downgrade control.}
...
\emph{auditable migration contract}:
\begin{itemize}\tightlist
\item \textbf{Explicit suite/provider separation} (the contract binds negotiation to a declared backend, not an implicit platform API default).
\item \textbf{Policy non-bypass at entry points} (policy-bound session API + compile/runtime guards block ad hoc fallback in caller glue).
\item \textbf{Narrow local-only fallback eligibility} (paired trust record, pre-send; timeout/network-derived causes are fallback-ineligible by definition) (D2).
\item \textbf{Downgrade-specific audit semantics} (replayable evidence fields + deterministic event typing distinguish downgrade vs failure) (D3).
\item \textbf{Explicit strictness realizations} (Compliance vs Bootstrap-Assisted strict recovery with mandatory PQC rekey gate before business traffic).
\end{itemize}
Supplementary Table~\ref{tab:novelty-delta} provides the full reviewer-facing prior-art vs.\ delta matrix and evidence anchors.
\textit{Ablation summary:} Supplementary Table~\ref{tab:ablation-by-property} provides an artifact-backed ablation-by-property map for the contract mechanisms above.
...
\textbf{Representative points in this design space:}
\begin{itemize}\tightlist
\item \textbf{TLS/QUIC deployment defaults} \cite{ref6,ref18}: strong handshake transcript integrity, but system-level retry and fallback causes and evidence are implementation-defined outside the protocol.
\item \textbf{Signal PQXDH} \cite{ref9}: hybrid key agreement at the application layer, but system-level fallback eligibility and replayable downgrade evidence are outside the protocol.
\item \textbf{SkyBridge migration contract (this work):} local-only fallback whitelist with timeout/network-derived blacklist, plus structured replayable downgrade evidence and deterministic event typing.
\end{itemize}
Supplementary Table~\ref{tab:migration-design-space} provides the full matrix.
...
\textbf{Evidence layering.} Our mechanized proofs establish security
properties for an abstract handshake state machine and its transcript
binding. Implementation conformance is supported by property-oriented
tests and audit-signal fidelity checks that map each proof obligation to
concrete code paths and artifacts (Supplementary
Table~\ref{tab:supp-formal-code-traceability}). Full code-level
verification of the Swift implementation is outside the scope of this
paper.
...
only occur for already paired peers and only for a whitelisted set of
locally generated \emph{pre-send} capability/instantiation errors;
timeout and network-derived failures never trigger default-mode
fallback.\\
\emph{Proof sketch:} Fallback is gated on a pinned trust record and a
pre-send whitelist evaluated at the initiator-side attempt boundary
(before emitting MessageA); whitelisted causes are restricted to stable local capability
predicates (Supplementary Table~\ref{tab:supp-default-fallback-eligibility}). \texttt{timeout}
is explicitly blocked and per-peer cooldown limits repeated downgrades.
...
\item \textbf{Tamper-evidence:} In addition to synchronous event emission, each session maintains a transcript-anchored audit hash-chain (\texttt{AuditTrail}). This provides cryptographic tamper evidence for in-session event order/content under non-compromised runtime assumptions. Long-term retention and remote attestation depend on the operating environment and are out of scope; privileged adversaries can truncate host logs.
\end{itemize}
...
\begin{itemize}\tightlist
\item \emph{We defend against:} (1) Attacker-induced timeout forcing downgrade (blocked by timeout blacklist); (2) Rapid downgrade cycling via repeated failures (per-peer cooldown; default 5 minutes); (3) Signature/identity attacks masquerading as capability failures (verification failures are blacklisted).
\item \emph{We do not defend against:} (1) Persistent PQC blocking under Default Mode (availability vs.\ security trade-off; Compliance Mode is fail-closed); (2) Legitimate PQC provider failures causing temporary classic fallback under Default Mode.
\item \emph{Cooldown rationale:} We set a conservative default of 5 minutes to bound downgrade cycling while limiting availability impact; the cooldown is a tunable operator knob (ablation in Supplementary Table~\ref{tab:migration-fleet-behavior}). Operators requiring zero classic establishment can select strict Compliance Mode.
\end{itemize}
...
\textbf{Evidence overview (main claims):}
\begin{itemize}\tightlist
\item Negotiation integrity and transcript binding: formal proof obligations + protocol/property tests (Section~\ref{sec:security-model}; \texttt{formal/skybridge\_v2.spthy}).
\item Policy-gated downgrade correctness: decision-matrix validation + forced-error policy bench + migration-threat harness contrast (Fig.~\ref{fig:policy-downgrade}; Fig.~\ref{fig:downgrade-matrix}; Table~\ref{tab:ablation-by-property}; Table~\ref{tab:migration-threat-harness}).
\item Deterministic failure semantics and event typing: fault injection + spec-to-event trace mapping (Supplementary Table~\ref{tab:failure-mode-robustness}; Table~\ref{tab:audit-signal-fidelity}).
\item Performance and overhead characterization: distributional microbench + system-level amortization (Table~\ref{tab:perf-summary}; Fig.~\ref{fig:system-impact-amortization}; Section~\ref{sec:system-impact}).
\item External-validity guardrail (deployment instance): controlled simulation + kernel cross-check + fleet dynamics + real-network micro-study (Section~\ref{sec:network-conditions}; Supplementary Tables~\ref{tab:supp-kernel-emulation}, \ref{tab:supp-ec2-direct}, \ref{tab:supp-interop-matrix}).
\end{itemize}
Supplementary Table~\ref{tab:claim-evidence-strength} provides the full claim-to-evidence map with artifact anchors.

\textbf{Threats to Validity (Reviewer-facing).}
\begin{itemize}\tightlist
\item \emph{Internal:} timing can be affected by runtime noise (scheduler/cache/thermal), so we use Release builds, warmup, distributional reporting, and multi-batch restarts.
\item \emph{Construct:} loopback \emph{time-to-ready} and microbench costs are not user QoE; we report \texttt{T\_connect}, \texttt{T\_first\_frame}, and \texttt{T\_total} separately.
\item \emph{External:} absolute costs are Apple-platform instance results; cross-stack evidence is framed as contract-consistency plus targeted Linux direct-path checks.
\item \emph{Conclusion:} audit-signal fidelity is deterministic spec-to-event conformance (not statistical); artifacts are published for independent reruns.
\end{itemize}
...
payload-only handshake baselines (classic \claimClassicPayloadBytes~B; core-gate PQC \claimLiboqsPayloadBytes~B for liboqs and \claimLiboqsVTwoPayloadBytes~B for liboqs v2 FS; Apple contrast \claimCryptoKitPayloadBytes~B where available),
...
SkyBridge Compass contributes an auditable migration contract for
crypto-agile P2P handshakes with explicit fallback semantics,
deterministic failure outcomes, and replayable evidence. In our Apple
deployment instance, default policy preserves availability with local-only
audited fallback, while strict policy remains fail-closed (or
bootstrap-assisted with mandatory PQC rekey). These are
deployment-instance measurements and should be re-established on
non-Apple targets.
