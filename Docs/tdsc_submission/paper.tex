...
and do not participate in hard pass/fail gates. We do not claim
cross-domain gate equivalence from a single monolithic script:
\emph{We separate source-quality, paper-integrity, and release-readiness
gates to avoid cross-domain false failures and to preserve
auditability.} To prevent upgrade-time moving-goalpost effects, this
track freezes wire formats, suite/wire IDs, canonical vectors, the
artifact date (\texttt{2026-01-23}), threat model, and strict/prefer/classic
policy semantics (including fallback-deny behavior). Cross-platform
...
\textbf{Why this is not ``TLS/QUIC + policy glue + logs'':} In widely
...
\item
  ...
  on-device measurements in Supplementary Table~\ref{tab:ios-microbench}; full
...
\item
  ...
  FS semantics (Supplementary Table~\ref{tab:supp-v2-v1-compare}).
\item
...
\noindent Additional directions are provided in the Supplementary Materials.
...
stability or operational transparency. We do not introduce new
cryptographic primitives; instead, we contribute an enforceable migration
contract (suite/provider separation, explicit policy semantics,
deterministic failure outcomes, and protocol-level audit signals) with
formalized definitions, machine-checkable proofs, and released artifacts.
Protocol v2 adds a forward-secrecy-oriented composition path while
remaining bridge-compatible with v1 deployments.

Our Apple-platform deployment indicates that native PQC APIs can be
integrated with modest overhead when available, with explicit, audited
fallback under Default Mode and fail-closed strict policy in two
operator-facing forms (Compliance and Bootstrap-Assisted). These results
are deployment-instance measurements and should be re-established when
porting to other ecosystems.
