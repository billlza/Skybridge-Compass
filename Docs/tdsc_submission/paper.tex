...
from \claimClassicPayloadBytes~B (classic) to \claimLiboqsPayloadBytes~B (liboqs PQC) or \claimCryptoKitPayloadBytes~B (CryptoKit PQC) in this snapshot. Beyond
...
The handshake payload size increases from Classic (\claimClassicPayloadBytes~B) to PQC (\claimLiboqsPayloadBytes~B for liboqs; \claimCryptoKitPayloadBytes~B for CryptoKit in this snapshot), dominated by ML-DSA signatures (3309~B) and ML-KEM ciphertexts (1088~B). Fig.~\ref{fig:message-size-breakdown} shows the per-field breakdown.
\textbf{Path-MTU implication.}
The payload-only PQC handshake (\claimLiboqsPayloadBytes~B for liboqs; \claimCryptoKitPayloadBytes~B for CryptoKit in this snapshot) spans multiple transport segments under common MTUs. In our design, handshake
...
payload-only handshake baselines (classic \claimClassicPayloadBytes~B; PQC \claimLiboqsPayloadBytes~B for liboqs and \claimCryptoKitPayloadBytes~B for CryptoKit in our snapshot),
