...
public sealed partial class WeatherBackdropDX : UserControl
{
    ...
    //    6 Haze    : Mac CinematicHazeView port - polluted-sky gradient, sun glow, fog banks, god rays, vignette
...
// scale is float2, not float: all three call sites pass float2(80,26)/(60,22)/(40,18) to get a
// non-uniform field (streaks tall and narrow). A scalar parameter made HLSL truncate that to .x
// and apply the horizontal figure to BOTH axes via `p *= scale`, which is what the three
// X3206 'implicit truncation of vector type' warnings were reporting - a real geometry bug,
// not just noise.
float rainLayer(float2 uv, float2 scale, float speed, float slant, float thickness, float seed)
{
    float2 p = uv;
...
// ============================ cinematic haze (Mac CinematicHazeView) ============================
// Port of the macOS haze stack: polluted-sky gradient + sun glow (CinematicHazeBackground),
// drifting fog banks (VolumetricHazeLayer, plusLighter), five god rays fanning from a sun just
// above the top edge (CinematicGodRaysLayer, screen) and an edge vignette (VignetteLayer).
// Deliberately not ported: CinematicFilmGrain (a per-frame 2px noise field, which under the
// glass panels reads as artefacting and would freeze into a fixed pattern when animation
// pauses), CinematicColorGrading (an overlay at 0.4 opacity whose strongest stop is 0.15, well
// under a 3% shift) and the sun glow's second gradient stop (folded into one smoothstep falloff).
// The Mac lays out rays and blobs in absolute points; here everything is in height units with an
// 800pt reference height so the scene scales with the window. The visible sun glow (0.7, 0.2) and
// the god-ray origin (0.75w, just above the top edge) are two independent points on the Mac too.
// ASCII ONLY (a non-ASCII char previously broke FXC).

// The finish stage of PSMain encodes with pow(saturate(col), 1.0 / 2.2); this is that exponent,
// so the decode below is its exact inverse. A follow-up can hoist the finish line onto it.
static const float DISPLAY_GAMMA = 2.2;

// Mac calculateHazeIntensity() default when no AQI reading is available. The cbuffer does not
// carry AQI yet, although the client already has it (WeatherSnapshot.Aqi, same 50/100/150/200/300
// buckets as the Mac table) and _pointerPad is a free slot for it; until that is plumbed the
// backdrop renders the Mac default for every air quality.
static const float HAZE_INTENSITY_DEFAULT = 0.6;

// Decodes a display-referred colour (a SwiftUI Color literal from the Mac sources) to the linear
// light this shader composes in. Gamma 2.2, matching the encode above, not the piecewise sRGB
// curve. Only the haze branch uses this: the other branches' constants were tuned by eye through
// the encode and are intentionally left raw. Any new colour copied from the Mac sources must go
// through this helper, otherwise it is encoded twice and washes out toward white.
float3 displayToLinear(float3 c) { return pow(saturate(c), DISPLAY_GAMMA); }

// Beam light from five god rays (linear, already weighted by opacity and intensity); the caller
// screen-blends it. Mac: origin (0.75w, -50px) with a slow sway, angles -25/-10/5/20/35 deg
// clockwise from +x in a y-down frame, quad widths 180/120/150/100/80px growing 0.3x -> 2.5x
// along the beam, opacities 0.25/0.35/0.30/0.20/0.15, and a gradient along the beam whose stops
// (1.0, 0.5, 0.2, 0) sit at 0, 1/3, 2/3 and 1 of the length. Widths are px / 800.
float3 hazeGodRays(float2 uv, float aspect, float intensity)
{
    static const float RAY_ANGLE[5]   = { -25.0, -10.0, 5.0, 20.0, 35.0 };
    static const float RAY_WIDTH[5]   = { 0.225, 0.150, 0.190, 0.125, 0.100 };
    static const float RAY_OPACITY[5] = { 0.25, 0.35, 0.30, 0.20, 0.15 };

    float2 p = float2(uv.x * aspect, uv.y);                 // height units, so beams keep their shape
    float2 origin = float2(0.75 * aspect + sin(time * 0.03) * 0.025,
                           -0.06 + cos(time * 0.02) * 0.012);
    float len = 1.5 * max(aspect, 1.0);
    float3 rayCol = displayToLinear(float3(1.0, 0.98, 0.90));
    float2 d = p - origin;
    ...
    {
        float k = (float)i;
        float ang = radians(RAY_ANGLE[i] + sin(time * 0.1 + k) * 2.0);
        float2 dir = float2(cos(ang), sin(ang));
        float along = dot(d, dir);
        float s = saturate(along / len);
        float halfW = lerp(RAY_WIDTH[i] * 0.3, RAY_WIDTH[i] * 2.5, s) * 0.5;
        float perp = abs(dot(d, float2(-dir.y, dir.x)));
        float beam = smoothstep(halfW, halfW * 0.7, perp) * step(0.0, along);
        float fade = pow(saturate(1.0 - s), 1.6);           // ~1 -> 0.5 -> 0.2 -> 0 along the beam
        float op = RAY_OPACITY[i] * intensity * (0.8 + 0.2 * sin(time * 0.15 + k * 0.5));
        acc += rayCol * beam * fade * op;
    }
    return acc;
}

// Mac VignetteLayer: radial gradient clear / clear / black@0.3v / black@0.6v with evenly spaced
// stops from 0.3*min(w,h) to 0.8*max(w,h). The Mac composites that black in display space, so the
// returned linear multiplier is (1-a)^gamma, which lands on the same darkening after the encode.
// It stacks on the light finish vignette in PSMain by design (the Mac scene has its own).
float hazeVignette(float2 uv, float aspect, float v)
{
    float r = length((uv - 0.5) * float2(aspect, 1.0));
    float startR = 0.3 * min(aspect, 1.0);
    float endR = 0.8 * max(aspect, 1.0);
    float s = saturate((r - startR) / (endR - startR));
    float a = v * (0.3 * saturate((s - 1.0 / 3.0) * 3.0) + 0.3 * saturate((s - 2.0 / 3.0) * 3.0));
    return pow(saturate(1.0 - a), DISPLAY_GAMMA);
}
...
float4 PSMain(VSOut input) : SV_TARGET
{
    ...
    {
        return finishFrame(frostedGlass(uv), uv, 1.0);
    }
    ...
    {
        // Mac CinematicHazeView parity. The haze scene is opaque at rest (Mac globalOpacity 1)
        // and the dark starry theme underneath shows only where a wave disperses it.
        float3 baseSky = nightSky(ty);
        baseSky += starfield(uv) * smoothstep(0.55, 0.0, ty) * 0.35;

        // 1. polluted atmosphere: yellow-brown top -> pale grey-yellow middle -> grey-brown bottom
        float3 skyTop = displayToLinear(float3(0.55, 0.50, 0.42));
        float3 skyMid = displayToLinear(float3(0.65, 0.60, 0.52));
        float3 skyLow = displayToLinear(float3(0.50, 0.48, 0.45));
        float3 haze = lerp(lerp(skyTop, skyMid, smoothstep(0.0, 0.5, ty)), skyLow, smoothstep(0.5, 1.0, ty));

        // sun glow through the particulate at (0.7 + 0.1 sin, 0.2), normal-blended like the Mac
        float2 sun = float2(0.7 + 0.1 * sin(time * 0.05), 0.2);
        float sd = length((uv - sun) * float2(aspect, 1.0));
        haze = lerp(haze, displayToLinear(float3(1.0, 0.95, 0.85)), smoothstep(0.32, 0.0, sd) * 0.3 * HAZE_INTENSITY_DEFAULT);

        // 2. drifting fog banks, additive (Mac plusLighter). Sampled at the pointer-warped UV so a
        //    wave pushes the banks aside as well as thinning them; aspect-corrected so they are
        //    not stretched on wide windows. The 0.42 weight was calibrated so the banks lift the
        //    frame mean by the same amount as a simulation of the Mac layer (about +0.07 display).
        float2 fogUV = dispUV * float2(aspect, 1.0);
        float f0 = fbm2(fogUV * 3.0 + float2(time * 0.02, 0.0), 4);
        float f1 = fbm2(fogUV * 5.0 - float2(time * 0.015, time * 0.01), 4);
        float f2 = fbm2(fogUV * 8.0 + float2(time * 0.01, time * 0.005), 3);
        float fog = smoothstep(0.35, 0.75, f0 * 0.5 + f1 * 0.3 + f2 * 0.2);
        haze += displayToLinear(float3(0.85, 0.82, 0.75)) * fog * (0.42 * HAZE_INTENSITY_DEFAULT);

        // 3. god rays, screen-blended at the Mac layer opacity (0.35 + 0.25 * intensity)
        float3 rays = hazeGodRays(uv, aspect, HAZE_INTENSITY_DEFAULT);
        haze = lerp(haze, haze + rays - haze * rays, 0.35 + 0.25 * HAZE_INTENSITY_DEFAULT);

        // 4. vignette (Mac intensity 0.3 + 0.2 * haze intensity)
        haze *= hazeVignette(uv, aspect, 0.3 + 0.2 * HAZE_INTENSITY_DEFAULT);

        col = lerp(baseSky, haze, densMul);                 // wave disperses the haze -> starry sky
        // scatterable motes in the Mac particle tint (MetalHazeParticleView), concentrated in the fog banks
        col += wispComposite(uv, aspect, fog, 0.22) * displayToLinear(float3(0.78, 0.72, 0.58));
    }
