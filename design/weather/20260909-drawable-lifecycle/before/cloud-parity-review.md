...
苹果两端的多云效果已移植，完整 Release 配置构建及相关测试通过。正式发布验收保持开放：iPhone 开启 Metal HUD 时仍有一条 drawable 生命周期提示，来源尚未定位，不能宣称全部运行时诊断已经清零。Windows 按后续要求暂缓。此次保留各端正式版安装，真机测试使用独立验证宿主。
...
1. iPhone 开启 Metal HUD 的日志出现：`[CAMetalLayerDrawable texture] should not be called after already presenting this drawable. Get a nextDrawable instead.` 五项断言及图像均通过，但尚无调用栈证明其根因。`iphone-native-cloud-pool.log` 保留该提示。
2. 在 iPad 对同一二进制仅向测试进程注入 `MTL_HUD_ENABLED=1` 后，五项测试通过，出现 HUD 重复统计项日志，但未复现上述 drawable 提示；默认 iPad 测试没有该提示。记录为 `ipad-hud-enabled.log` 和 `ipad-metal-hud-diagnostic.log`。未修改设备全局 HUD 设置。这一对照不足以宣称 iPhone 提示已解决，需其回连后进一步定位。
