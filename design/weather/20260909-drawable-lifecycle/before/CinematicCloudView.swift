...
/// Opaque, bounded-resolution cloud sky. Only the native surface owns frame cadence.
public struct CinematicCloudView: View {
    ...
    @State private var renderer: CloudVolumeRenderer?
    ...
    public var body: some View {
        Group {
            if failure != nil {
                ...
                CloudNativeView(renderer: renderer, intensity: intensity, wind: wind, quality: quality,
                                framesPerSecond: framesPerSecond, isAnimating: isAnimating, onFailure: recordFailure)
            } else {
        ...
        .task {
            ...
            do {
                let loaded = try await CloudVolumeRenderer.load()
    ...
    private func recordFailure(_ message: String) {
        Logger(subsystem: "com.skybridge.weather", category: "CloudRendering").error("\(message, privacy: .public)")
        failure = message
    }
}

private struct CloudNativeView {
    let renderer: CloudVolumeRenderer
    let intensity: Float
    let wind: Float
    let quality: Float
    ...
    func makeSurface(coordinator: Coordinator) -> MTKView {
        let view = CloudMetalSurface(frame: .zero, device: renderer.device)
    ...
    final class Coordinator: NSObject, MTKViewDelegate {
        var settings: CloudNativeView
        private var clock = CloudAnimationClock()
        private let availableFrames = DispatchSemaphore(value: 2)
        private var needsFrame = true

        init(settings: CloudNativeView) { self.settings = settings }

        func update(_ settings: CloudNativeView, view: MTKView) {
            if self.settings.intensity != settings.intensity || self.settings.wind != settings.wind ||
                self.settings.quality != settings.quality {
                needsFrame = true
            }
            self.settings = settings
            if !settings.isAnimating { clock.pause() }
            view.preferredFramesPerSecond = settings.framesPerSecond
            view.isPaused = !settings.isAnimating
            resize(view)
            if view.isPaused && needsFrame { view.draw() }
        }
        ...
        func resize(_ view: MTKView) {
            ...
            let size = CloudRenderPolicy.drawableSize(for: pixels, quality: settings.quality)
        ...
        private func renderFrame(in view: MTKView) {
            ...
            // Let MetalKit acquire/configure the frame attachment before obtaining its drawable.
            ...
            do {
                guard let buffer = settings.renderer.queue.makeCommandBuffer() else {
                    throw CloudRenderError.unavailable("command buffer allocation failed")
                }
                let time = clock.sample(at: CACurrentMediaTime(), animating: settings.isAnimating)
                let uniforms = CloudUniforms(resolution: SIMD2(Float(view.drawableSize.width), Float(view.drawableSize.height)),
...
@MainActor
private final class CloudMetalSurface: MTKView {
    var onLayout: (() -> Void)?
    #if os(macOS)
...
#if os(macOS)
extension CloudNativeView: NSViewRepresentable {
...
#else
extension CloudNativeView: UIViewRepresentable {
