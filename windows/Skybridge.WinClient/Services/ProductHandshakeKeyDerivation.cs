...
/// </summary>
public static class ProductHandshakeKeyDerivation
{
    public const int SharedSecretLength = 32;
    public const int TranscriptHashLength = 32;
    public const int NonceLength = 32;

    public static ProductSessionKeys Derive(
        ...
        ProductHandshakeRole role)
    {
        RequireLength(sharedSecret, SharedSecretLength, "Product handshake shared secret");
        RequireLength(transcriptA, TranscriptHashLength, "Product handshake transcriptA");
        RequireLength(transcriptB, TranscriptHashLength, "Product handshake transcriptB");
        RequireLength(clientNonce, NonceLength, "Product handshake client nonce");
        RequireLength(serverNonce, NonceLength, "Product handshake server nonce");
        ...
        try
        {
            return role switch
            {
                ProductHandshakeRole.Initiator => new ProductSessionKeys(
                    role,
                    DeterministicSessionId(transcriptHash),
                    transcriptHash,
                    i2rKey,
                    r2iKey),
                ProductHandshakeRole.Responder => new ProductSessionKeys(
                    role,
                    DeterministicSessionId(transcriptHash),
                    transcriptHash,
                    r2iKey,
                    i2rKey),
                _ => throw new InvalidDataException($"Unsupported Product handshake role '{role}'.")
            };
        }
        finally
        {
            CryptographicOperations.ZeroMemory(i2rKey);
            CryptographicOperations.ZeroMemory(r2iKey);
            CryptographicOperations.ZeroMemory(transcriptHash);
        }
    }

    public static WebRtcProductHandshakeFinished CreateFinished(ProductSessionKeys keys)
    {
        ArgumentNullException.ThrowIfNull(keys);
        var direction = keys.Role switch
        {
            ProductHandshakeRole.Initiator => WebRtcProductHandshakeFinishedDirection.InitiatorToResponder,
            ProductHandshakeRole.Responder => WebRtcProductHandshakeFinishedDirection.ResponderToInitiator,
    ...
    public static bool VerifyFinished(
        ...
        ProductSessionKeys keys,
        ProductHandshakeRole expectingFrom)
    {
    ...
    {
        RequireLength(transcriptHash, TranscriptHashLength, "Product handshake transcript hash");
    ...
    {
        RequireLength(baseKey, 32, "Product handshake Finished base key");
        RequireLength(transcriptHash, TranscriptHashLength, "Product handshake Finished transcript hash");
        var label = direction switch
        {
            ...
            _ => throw new InvalidDataException($"Unsupported Product handshake Finished direction '{direction}'.")
        };
    ...
    {
        WebRtcProductHandshakeCodec.RequireKnownSuite(suiteWireId);
        return suiteWireId switch
        {
            WebRtcProductHandshakeCodec.SuiteQPeriaptPolicyBound => "qperiapt-abi2-policy-bound-v1",
            WebRtcProductHandshakeCodec.SuiteMlKem768Mldsa65ForwardSecure => "v2-static+ephemeral",
            _ => "v1-single"
        };
    }
