...
enum CrossNetworkWebRTCTraceDescription {
    ...
    static func describeEnvelope(_ envelope: WebRTCSignalingEnvelope) -> String {
        ...
        return "session_ref=\(SkyBridgeDiagnosticReference.stableReference(envelope.sessionId)) type=\(envelope.type.rawValue) from_ref=\(SkyBridgeDiagnosticReference.stableReference(envelope.from)) to_ref=\(SkyBridgeDiagnosticReference.stableReference(envelope.to)) auth=\(envelope.authToken == nil ? 0 : 1) \(payloadSummary)"
    }
