...
///
/// This is mirrored from macOS `SkyBridgeCore` so both sides stay byte-for-byte compatible.
@available(iOS 17.0, *)
...
public struct CrossNetworkFileTransferCompletionAckExpectation: Equatable, Sendable {
    ...
    public func validate(_ message: CrossNetworkFileTransferMessage) throws {
        ...
        guard actualFileSHA256.count == 32 else {
            throw CrossNetworkFileTransferCompletionEvidenceError.invalidFileSHA256Length(actualFileSHA256.count)
        }
...
public struct CrossNetworkFileTransferWaiterKey: Hashable, Sendable {
    ...
    public init(owner: CrossNetworkFileTransferSessionOwner, transferID: String, operation: CrossNetworkFileTransferOp, chunkIndex: Int?) throws {
        try CrossNetworkInboundFileCommitter.requireCanonicalTransferID(transferID)
        self.owner = owner
        self.transferID = transferID
...
public struct CrossNetworkFileTransferWaiterRegistry: Sendable {
    ...
    public mutating func arm(owner: CrossNetworkFileTransferSessionOwner, transferID: String, operation: CrossNetworkFileTransferOp, chunkIndex: Int?) throws -> CrossNetworkFileTransferWaiterToken {
        let key = try CrossNetworkFileTransferWaiterKey(owner: owner, transferID: transferID, operation: operation, chunkIndex: chunkIndex)
    ...
    public mutating func consume(owner: CrossNetworkFileTransferSessionOwner, message: CrossNetworkFileTransferMessage) -> CrossNetworkFileTransferWaiterToken? {
        guard let key = try? CrossNetworkFileTransferWaiterKey(owner: owner, transferID: message.transferId, operation: message.op, chunkIndex: message.chunkIndex),
              let tokenID = tokenIDByKey.removeValue(forKey: key) else { return nil }
    ...
    public mutating func remove(owner: CrossNetworkFileTransferSessionOwner, transferID: String) -> [CrossNetworkFileTransferWaiterToken] {
        let matchingKeys = tokenIDByKey.keys.filter { $0.owner == owner && $0.transferID == transferID }
...
/// Bounded process-local Level 1 reliability witness; it is not crash-durable state.
public struct CrossNetworkFileTransferCompletionReplayCache: Sendable {
    ...
    private enum Entry: Sendable {
        case active
        case completed(fingerprint: CrossNetworkFileTransferCompletionRequestFingerprint, acknowledgement: CrossNetworkFileTransferMessage, completedAt: Date)
    ...
    public mutating func reserve(owner: CrossNetworkFileTransferSessionOwner, transferID: String, now: Date = Date()) throws -> CrossNetworkFileTransferReplayReservation {
        try CrossNetworkInboundFileCommitter.requireCanonicalTransferID(transferID)
        pruneExpired(now: now)
        let key = Key(owner: owner, transferID: transferID)
        if let entry = entries[key] {
            switch entry {
            case .active: return .alreadyActive
            case .completed, .completedTombstone: return .alreadyCompleted
            }
        }
    ...
    public mutating func recordCompletion(owner: CrossNetworkFileTransferSessionOwner, fingerprint: CrossNetworkFileTransferCompletionRequestFingerprint, acknowledgement: CrossNetworkFileTransferMessage, now: Date = Date()) throws {
        let prepared = try prepareCompletion(owner: owner, fingerprint: fingerprint, acknowledgement: acknowledgement)
    ...
    public func prepareCompletion(owner: CrossNetworkFileTransferSessionOwner, fingerprint: CrossNetworkFileTransferCompletionRequestFingerprint, acknowledgement: CrossNetworkFileTransferMessage) throws -> CrossNetworkFileTransferPreparedCompletion {
        let key = Key(owner: owner, transferID: fingerprint.transferID)
        guard case .active? = entries[key] else {
            throw CrossNetworkFileTransferCompletionReplayError.missingActiveReservation
        }
        let expectation = try CrossNetworkFileTransferCompletionAckExpectation(transferID: fingerprint.transferID, receivedBytes: fingerprint.receivedBytes, fileSHA256: fingerprint.fileSHA256)
        do {
        ...
        return CrossNetworkFileTransferPreparedCompletion(owner: owner, fingerprint: fingerprint, acknowledgement: acknowledgement)
    }

    @discardableResult
    public mutating func commitPreparedCompletion(_ prepared: CrossNetworkFileTransferPreparedCompletion, now: Date = Date()) -> Bool {
        let key = Key(owner: prepared.owner, transferID: prepared.fingerprint.transferID)
        guard case .active? = entries[key] else { return false }
        entries[key] = .completed(fingerprint: prepared.fingerprint, acknowledgement: prepared.acknowledgement, completedAt: now)
        return true
    }

    public mutating func lookup(owner: CrossNetworkFileTransferSessionOwner, fingerprint: CrossNetworkFileTransferCompletionRequestFingerprint, now: Date = Date()) -> CrossNetworkFileTransferCompletionReplayLookup {
        pruneExpired(now: now)
        let key = Key(owner: owner, transferID: fingerprint.transferID)
        ...
        case .completedTombstone: return .tombstone
        }
    }

    public mutating func releaseActive(owner: CrossNetworkFileTransferSessionOwner, transferID: String) {
        let key = Key(owner: owner, transferID: transferID)
    ...
    private mutating func pruneExpired(now: Date) {
        entries = entries.mapValues { entry in
            ...
            case .completed(_, _, let completedAt):
                return now.timeIntervalSince(completedAt) < timeToLive ? entry : .completedTombstone
            case .completedTombstone: return .completedTombstone
            }
        }
    }
}
