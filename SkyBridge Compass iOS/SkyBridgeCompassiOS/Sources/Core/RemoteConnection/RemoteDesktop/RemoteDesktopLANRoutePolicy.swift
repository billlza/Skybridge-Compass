    ...
    static func resolvedRouteRejection(
        ...
        if case .hostPort(let host, _) = requestedEndpoint,
           ConnectableAddressCanonicalizer.prefersPeerToPeer(for: String(describing: host)) {
            return "resolved peer-to-peer remote route rejected: requested=\(String(describing: requestedEndpoint))"
        }

        guard let resolvedEndpoint else {
            if case .service = requestedEndpoint {
                return "unverified Bonjour remote route rejected: requested=\(String(describing: requestedEndpoint))"
            }
            return nil
        }

        guard case .hostPort(let resolvedHost, _) = resolvedEndpoint else {
            return "unverified remote route rejected: requested=\(String(describing: requestedEndpoint)) resolved=\(String(describing: resolvedEndpoint))"
        }
        ...
        if ConnectableAddressCanonicalizer.prefersPeerToPeer(for: resolvedHostText) {
            return "resolved peer-to-peer remote route rejected: requested=\(String(describing: requestedEndpoint)) resolved=\(String(describing: resolvedEndpoint))"
        }

        if case .service = requestedEndpoint,
           !ConnectableAddressCanonicalizer.isRoutableLANAddress(resolvedHostText) {
            return "unverified Bonjour remote route rejected: requested=\(String(describing: requestedEndpoint)) resolved=\(String(describing: resolvedEndpoint))"
        }
        return nil
    }
