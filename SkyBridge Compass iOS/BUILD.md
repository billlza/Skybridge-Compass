...
4. 使用 `Scripts/export_ios_app_store_product.sh` 从同一 archive 以 `app-store-connect` 方法导出；固定 `manageAppVersionAndBuildNumber=false`，不得让上传工具改写仓库声明的 `1.0.2 (3)`。导出器不会上传。
