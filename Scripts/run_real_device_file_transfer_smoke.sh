...
REQUIRE_SIGNED_KEM_REFRESH="${SKYBRIDGE_SMOKE_REQUIRE_SIGNED_KEM_REFRESH:-0}"
...
REQUIRE_MAC_INITIATED_RECONNECT="${SKYBRIDGE_SMOKE_REQUIRE_MAC_INITIATED_RECONNECT:-1}"
...
SMOKE_BUILD_DIR="${SKYBRIDGE_FILE_TRANSFER_SMOKE_BUILD_DIR:-$ROOT_DIR/.build/real-device-file-transfer-smoke}"
...
case "$ALLOW_PERSISTENT_TRUST_MUTATION" in
  0|1) ;;
  *)
    echo "Unsupported SKYBRIDGE_SMOKE_ALLOW_PERSISTENT_TRUST_MUTATION=$ALLOW_PERSISTENT_TRUST_MUTATION (expected 0 or 1)" >&2
    exit 2
    ;;
esac

if [[ "$REQUIRE_SIGNED_KEM_REFRESH" != "$FORCE_SIGNED_KEM_REFRESH" ]]; then
...
if [[ "$FORCE_SIGNED_KEM_REFRESH" == "1" && "$ALLOW_PERSISTENT_TRUST_MUTATION" != "1" ]]; then
...
echo "==> Force signed KEM refresh by clearing KEM cache: $FORCE_SIGNED_KEM_REFRESH (explicit trust mutation approval: $ALLOW_PERSISTENT_TRUST_MUTATION)"
