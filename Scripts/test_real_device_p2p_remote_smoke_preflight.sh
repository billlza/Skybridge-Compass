...
[[ -n "$preflight_line" && -n "$build_line" ]] \
  || fail "smoke script should contain both preflight and build markers"
...
contains_literal "$mac_online_build_body" "/usr/bin/codesign --force --deep --sign - \"\$MAC_ONLINE_APP_BUNDLE\"" \
  || fail "Mac online iPad Debug app must be re-signed before LaunchServices open"
