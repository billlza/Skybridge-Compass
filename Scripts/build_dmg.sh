...
if [[ "$USE_EXISTING_APP" == true ]]; then
    ...
    if [[ -n "$SIGNING_IDENTITY" ]]; then
        ...
        SKYBRIDGE_XCODE_APP_BUNDLE="$XCODE_APP_BUNDLE" SKYBRIDGE_PACKAGE_MAIN_BUILD_SYSTEM="$MAIN_BUILD_SYSTEM" SKYBRIDGE_SWIFTPM_RELEASE_SCRATCH_PATH="${SKYBRIDGE_SWIFTPM_RELEASE_SCRATCH_PATH:-}" SKIP_BUILD=1 ALLOW_STALE_BUILD="$PACKAGE_APP_ALLOW_STALE_BUILD" SKYBRIDGE_PACKAGE_CONTEXT=release_dmg IDENTITY="$SIGNING_IDENTITY" "$PROJECT_ROOT/Scripts/package_app.sh"
    else
