...
case "${TOOLCHAIN_POLICY}" in
  stable-release)
    # Established shipping line. Exact-match the Xcode toolchain and shipped SDK
    # facts, reject beta dirs.
  ...
  os27-release)
    ...
    ;;
  custom-diagnostic)
    EXPECTED_XCODE_VERSION="${SKYBRIDGE_REQUIRED_XCODE_VERSION:-26.6}"
    EXPECTED_XCODE_BUILD="${SKYBRIDGE_REQUIRED_XCODE_BUILD:-17F113}"
