...
for required_check in \
  ...
  macos-update-publish-transaction; do
  /usr/bin/grep -Fq -- "\"${required_check}\"" "${SOURCE_QUALITY_GATE}" || {
    echo "source-quality gate is missing required check: ${required_check}" >&2
    exit 1
  }
done
