...
def fig_system_impact_amortization():
    """
    Figure: System-level impact + amortization.
    Inputs: Artifacts/system_impact_<date>.csv (generated by SystemImpactBenchTests)
    """
    rows = _read_csv_dicts(_artifact_csv("system_impact"))

    # Helpers
    def _group(rows, condition: str, suite: str):
        return [r for r in rows if r["condition"] == condition and r["suite"] == suite]

    def _finite_floats(rows, key: str):
        out = []
        for r in rows:
            try:
    ...
    conds = [
        ("ideal", "ideal"),
        ("rtt50_j20", "RTT 50±20ms"),
        ("rtt100_j50", "RTT 100±50ms"),
    ]

    fig, (ax1, ax2) = plt.subplots(
        2, 1, figsize=(COL_WIDTH, 3.2), gridspec_kw={"height_ratios": [1.0, 1.2]}
    )

    # --- Panel A: T_connect p95 by condition ---
    x = np.arange(len(conds))
    width = 0.24
    for i, (suite_id, suite_label) in enumerate(suites):
        y = []
        for cond_id, _ in conds:
            vals = _finite_floats(_group(rows, cond_id, suite_id), "t_connect_ms")
            y.append(_pctl(vals, 0.95))
        ax1.bar(x + (i - 1) * width, y, width=width, label=suite_label,
                color={"classic": COLORS["classic"], "pqc_liboqs": COLORS["liboqs"], "pqc_cryptokit": COLORS["cryptokit"]}[suite_id])
    ax1.set_xticks(x)
    ax1.set_xticklabels([label for _, label in conds])
    ax1.set_ylabel("T_connect p95 (ms)")
    ax1.grid(axis="y", alpha=0.25)
    ax1.legend(loc="upper left", ncols=3, frameon=False)
    ax1.set_title("System-level impact: connect + amortization (p95)")

    # --- Panel B: Amortization curve (total time p95 vs file size) under RTT 50±20ms ---
    cond_id = "rtt50_j20"
    # discover file sizes
    file_sizes = sorted({int(r["file_bytes"]) for r in rows if r["condition"] == cond_id})
    x_mb = np.array([b / (1024 * 1024) for b in file_sizes], dtype=float)
    for suite_id, suite_label in suites:
        y = []
        for fb in file_sizes:
            vals = _finite_floats(
                [r for r in rows if r["condition"] == cond_id and r["suite"] == suite_id and int(r["file_bytes"]) == fb],
                "t_file_total_ms",
            )
            y.append(_pctl(vals, 0.95))
        ax2.plot(x_mb, y, marker="o", label=suite_label,
                 color={"classic": COLORS["classic"], "pqc_liboqs": COLORS["liboqs"], "pqc_cryptokit": COLORS["cryptokit"]}[suite_id])
    ax2.set_xlabel("File size (MiB)")
    ax2.set_ylabel("T_total p95 (ms)\n(connect + transfer)")
    ax2.grid(alpha=0.25)
    ax2.legend(loc="upper left", ncols=3, frameon=False)
...
def main():
    ...
    if os.environ.get("SKYBRIDGE_ONLY_SYSTEM_IMPACT_FIG") == "1":
        fig_system_impact_amortization()
        print()
        print("System impact figure generated successfully!")
        return
    ...
    # Optional (depends on running SystemImpactBenchTests first)
    try:
        fig_system_impact_amortization()
    except Exception as e:
        print(f"[WARN] Skipping system impact figure: {e}")
