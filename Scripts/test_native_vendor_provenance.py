...
class NativeVendorProvenanceTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.recipe_input_path = self.repository_root / "Scripts/native_helper.sh"
        ...
        self.recipe_input_path.write_text("helper input\n", encoding="utf-8")
        ...
        self.lock = {
            ...
            "families": {
                "example": {
                    ...
                    "recipe_inputs": {
                        "Scripts/native_helper.sh": PROVENANCE.sha256_file(
                            self.recipe_input_path
                        ),
                    },
                    "toolchain": {
                        "xcode": "fixture-xcode",
            ...
            "native_dependency_lock": {
                ...
                "family_sha256": PROVENANCE.locked_family_sha256(
                    "example",
                    {
                        ...
                        "recipe_inputs": self.lock["families"]["example"]["recipe_inputs"],
    ...
    def test_recipe_input_record_that_differs_from_lock_is_rejected(self) -> None:
        record = self.record()
        record["build"]["recipe_inputs"]["Scripts/native_helper.sh"] = "f" * 64
        with self.assertRaisesRegex(PROVENANCE.ProvenanceError, "recipe inputs differ"):
            self.verify(record)
    ...
    def test_recipe_input_tampering_is_rejected(self) -> None:
        record = self.record()
        self.recipe_input_path.write_text("tampered helper\n", encoding="utf-8")
        with self.assertRaisesRegex(PROVENANCE.ProvenanceError, "recipe input differs"):
            self.verify(record)

    def test_recipe_input_symlink_is_rejected(self) -> None:
        record = self.record()
        real_input = self.recipe_input_path.with_name("real_native_helper.sh")
        self.recipe_input_path.rename(real_input)
        self.recipe_input_path.symlink_to(real_input.name)
