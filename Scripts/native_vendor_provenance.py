...
BUILD_FIELDS = {"inputs", "recipe", "recipe_sha256", "toolchain"}
...
def load_locked_family(
    ...
    if set(locked_family) != {"sources", "build_inputs", "toolchain"}:
        raise ProvenanceError(
...
def create_record(args: argparse.Namespace) -> dict[str, Any]:
    ...
    assert_matches_locked_family(
        args.family, sources, build_inputs, toolchain, locked_family
    )
    roots: dict[str, Any] = {}
