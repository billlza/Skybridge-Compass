...
require_literal 'skybridge_archive_ios_distribution_product' "$WEBRTC_SCRIPT"
require_literal 'skybridge_export_ios_distribution_archive' "$WEBRTC_SCRIPT"
require_literal 'skybridge_extract_single_ios_exported_app' "$WEBRTC_SCRIPT"
require_literal 'installed-only' "$WEBRTC_SCRIPT"
...
require_literal 'unset CODE_SIGN_IDENTITY' "$IOS_DISTRIBUTION_SIGNING_HELPERS"
require_literal 'IPA contains an unsafe path component' "$IOS_IPA_EXTRACTOR"
require_literal 'IPA contains a link or special file' "$IOS_IPA_EXTRACTOR"
require_literal 'IPA contains duplicate normalized paths' "$IOS_IPA_EXTRACTOR"
...
require_literal '"SKYBRIDGE_PACKAGING_SOURCE_REPOSITORY=${GITHUB_REPOSITORY:-${SKYBRIDGE_SOURCE_REPOSITORY:-billlza/Skybridge-Compass}}"' "$P2P_SCRIPT"
require_literal '"SKYBRIDGE_PACKAGING_PRODUCT_SURFACE=testing"' "$P2P_SCRIPT"
require_literal '"SKYBRIDGE_PACKAGING_SWIFT_ACTIVE_COMPILATION_CONDITIONS=HAS_APPLE_PQC_SDK,SKYBRIDGE_TESTING"' "$P2P_SCRIPT"
require_literal '"OTHER_SWIFT_FLAGS=\$(inherited) -D SKYBRIDGE_TESTING"' "$P2P_SCRIPT"
require_literal '"SKYBRIDGE_PACKAGING_SOURCE_REPOSITORY=${GITHUB_REPOSITORY:-${SKYBRIDGE_SOURCE_REPOSITORY:-billlza/Skybridge-Compass}}"' "$WEBRTC_SCRIPT"
...
require_literal 'skybridge_smoke_check_public_artifacts "$PUBLIC_ARTIFACT_DIR" "$IOS_DEVICE_ID" "$MAC_DEVICE_ID" "$IOS_LOGICAL_DEVICE_ID" "$MAC_PQC_DEVICE_ID"' "$WEBRTC_SCRIPT"
webrtc_archive_line="$(line_number 'skybridge_archive_ios_distribution_product' "$WEBRTC_SCRIPT")"
webrtc_export_line="$(line_number 'skybridge_export_ios_distribution_archive' "$WEBRTC_SCRIPT")"
webrtc_extract_line="$(line_number 'skybridge_extract_single_ios_exported_app' "$WEBRTC_SCRIPT")"
webrtc_product_proof_line="$(line_number 'verify_ios_product_app "$IOS_APP_PATH"' "$WEBRTC_SCRIPT")"
webrtc_install_line="$(line_number 'device install app --device "$IOS_DEVICE_ID" "$IOS_APP_PATH"' "$WEBRTC_SCRIPT")"
[[ -n "$webrtc_archive_line" && -n "$webrtc_export_line" && -n "$webrtc_extract_line" && -n "$webrtc_product_proof_line" && -n "$webrtc_install_line" ]] \
  || fail "WebRTC Release path must contain archive, export, safe extraction, product proof, and install stages"
(( webrtc_archive_line < webrtc_export_line && webrtc_export_line < webrtc_extract_line && webrtc_extract_line < webrtc_product_proof_line && webrtc_product_proof_line < webrtc_install_line )) \
  || fail "WebRTC Release path must prove archive -> export -> safe extraction -> product proof before install"
