...
struct WebRTCSessionFramedPayloadValidationTests {
    ...
    func realtimeMediaAudioRefreshesAdmissionAndRelayLeaseExpiry() throws {
        ...
        #expect(viewerSource.contains("let strictRenewalRequiresRollover = strictRenewalRequired && sameRelayAddress"))
        #expect(viewerSource.contains("let strictRenewalRequired = Self.shouldRequestExtremeMediaValidation("))
        ...
        #expect(viewerSource.contains("guard let crossNetworkOwner = owner.transportFailureOwner.crossNetworkOwner"))
        #expect(viewerSource.contains("relayEndpointPair = try await crossNetwork.requestRealtimeMediaRelayEndpoint("))
        #expect(viewerSource.contains("for: crossNetworkOwner"))
    ...
    func streamConfigurationAckAndNonblockingAudioReceiverStartupAreWired() throws {
        ...
        let sharedTransactionSource = try readSource(
            "Sources/SkyBridgeProtocolCore/RemoteConnection/RemoteDesktopStreamConfigurationTransaction.swift"
        )
        let viewerSource = try readSource("SkyBridge Compass iOS/SkyBridgeCompassiOS/Sources/Managers/RemoteDesktopManager.swift")
        ...
        #expect(hostWireSource.contains("import SkyBridgeProtocolCore"))
        #expect(sharedTransactionSource.contains("RemoteDesktopStreamConfigurationAcknowledgement"))
        #expect(sharedTransactionSource.contains("RemoteDesktopStreamConfigurationTransaction"))
        ...
        #expect(viewerTypesSource.contains("import SkyBridgeProtocolCore"))
        #expect(viewerSource.contains("currentRealtimeMediaAudioBindingIfUsable("))
        #expect(viewerSource.contains("expectedMode: mediaAudioMode"))
        ...
        #expect(viewerSource.contains("scheduleStreamConfigurationAckRetryIfNeeded("))
        #expect(viewerSource.contains("for: payload,"))
        #expect(viewerSource.contains("retryAttempt: index + 1,"))
        #expect(viewerSource.contains("operationWitness: operationWitness"))
        #expect(viewerSource.contains("transportOwner: transportOwner"))
        #expect(viewerSource.contains("payload.streamConfigurationTransaction == owner.transaction"))
        #expect(viewerSource.contains("isCurrentViewerStreamConfigurationSendOwner(owner)"))
        #expect(viewerSource.contains("acknowledgementMatches("))
