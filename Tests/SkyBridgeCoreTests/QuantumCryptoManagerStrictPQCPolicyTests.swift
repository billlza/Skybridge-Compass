...
final class QuantumCryptoManagerStrictPQCPolicyTests: XCTestCase {
    ...
    func testPairingIdentityExchangeKEMBootstrapFailsClosedWhenTrustSyncPersistenceFails() throws {
        ...
        XCTAssertTrue(
            ...
            "The pairing transaction must make TrustSync authority its final commit point."
        )
        XCTAssertTrue(
            commitBody.contains("guard persisted else") &&
                commitBody.contains("throw CommitError.authorityNotPromoted"),
            "Failed authority promotion must remain an explicit transaction failure."
        )
        XCTAssertTrue(
            commitBody.contains("rollbackAuthorityBoundPairingKEMMutation(kemReceipt)"),
            "Pre-commit failure must conditionally roll back only its exact staged KEM receipt."
        )
        XCTAssertFalse(
        ...
        XCTAssertFalse(commitBody.contains("clearPairingIdentityExchangeEntries("))
        XCTAssertFalse(commitBody.contains("PeerKEMBootstrapStore.shared.clear("))
    }
    ...
    func testPQCProviderFactoryRequiresSuiteSpecificXWingHPKEProbe() throws {
        ...
        XCTAssertTrue(
            makeProviderBody.contains("case .hybridXWing:") &&
                makeProviderBody.contains("return makeHPKEProvider(") &&
                makeProviderBody.contains("for: .hybridXWing"),
        ...
        XCTAssertTrue(
            source.contains("return ApplePQCProvider(") &&
                source.contains("suite: .hybridXWing"),
            "X-Wing factory output must carry hybridXWing suite metadata."
        )
        XCTAssertTrue(
            source.contains("suite: .pqcMlKemMlDsa") &&
                source.contains("keyPairAuthority: .active"),
    ...
    func testPQCProviderKeychainPersistenceFailuresFailClosedAndRedactIdentifiers() throws {
        ...
        XCTAssertTrue(providerSource.contains("PQCKeyPairStore.loadOrCreate("))
        XCTAssertTrue(providerSource.contains("PQCKeyPairStore.insertIfAbsent("))
        XCTAssertTrue(providerSource.contains("canonicalRecordMissingAfterInsert"))
        XCTAssertFalse(
        ...
        XCTAssertTrue(oqsBridgeSource.contains("PQCKeyPairStore.loadOrCreate("))
        XCTAssertTrue(oqsBridgeSource.contains("PQCKeyPairStore.loadOrMigrateLegacy("))
        XCTAssertFalse(
