...
struct FileTransferSignalingServiceTests {
 ...
 // 传输不应被添加
    ...
    @Test("处理文件元数据 - 无显式策略默认拒绝")
    @MainActor
    func testHandleFileMetaFailsClosedWithoutApprovalHandler() async {
        guard #available(macOS 14.0, *) else { return }
        ...
        #expect(ackMessage.accepted == false)
        #expect(ackMessage.reason == "explicit_file_receive_approval_required")
        #expect(service.activeTransfers[incomingMeta.fileId] == nil)
    }
 ...
 // 验证回调
    ...
    @Test("处理文件结束消息 - 成功但字节不一致应失败")
    @MainActor
    func testHandleFileEndSuccessWithByteMismatchFailsClosed() async {
        guard #available(macOS 14.0, *) else { return }
        ...
        var completedSuccess: Bool?
        service.onFileTransferCompleted = { _, success in
            completedSuccess = success
        }
