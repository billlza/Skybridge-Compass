...
final class SettingsRuntimeTruthSourceContractTests: XCTestCase {
    ...
    func testIOSBonjourAdvertisesProtocolIdentityNotLegacyMobileIdentity() throws {
        ...
        XCTAssertTrue(discovery.contains("BonjourInteropProtocolContract.canonicalAdvertisementFields("))
        XCTAssertTrue(discovery.contains("role: .control"))
        XCTAssertTrue(discovery.contains("let fields = try Self.primaryBonjourInteropAdvertisementFields("))
        XCTAssertTrue(discovery.contains("for (key, value) in fields"))
