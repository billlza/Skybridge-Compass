...
final class SettingsRuntimeTruthSourceContractTests: XCTestCase {
    ...
    func testRemoteDesktopSettingsReachP2PAndWebRTCRuntimeConsumers() throws {
        ...
        XCTAssertTrue(rdpManager.contains("try client.applyAllSettings(dict)"))
        XCTAssertTrue(rdpManager.contains("\"colorDepth\": settings.displaySettings.colorDepth.rawValue"))
        XCTAssertTrue(rdpManager.contains("\"connectionType\": settings.networkSettings.connectionType.rawValue"))
        XCTAssertFalse(rdpManager.contains("\"compressionLevel\": settings.displaySettings.boundedCompressionLevelPercent"))
        XCTAssertFalse(rdpManager.contains("\"connectionTimeout\": settings.networkSettings.boundedConnectionTimeoutSeconds * 1_000"))
        XCTAssertFalse(rdpManager.contains("\"enableNetworkStats\": settings.networkSettings.enableNetworkStats"))
    ...
    func testStrictPQCSecuritySourcesDoNotExposeFakeSuccessFallbacks() throws {
        ...
        XCTAssertFalse(pake.contains("fatalError("))
        XCTAssertFalse(pake.contains("SecRandomCopyBytes"))
    ...
    func testIOSBonjourAdvertisesProtocolIdentityNotLegacyMobileIdentity() throws {
        ...
        XCTAssertTrue(discovery.contains("let txtRecord = try createTXTRecord(authority: authority)"))
        ...
        XCTAssertTrue(discovery.contains("BonjourInteropProtocolContract.canonicalAdvertisementWireData("))
        XCTAssertTrue(discovery.contains("role: .control"))
        XCTAssertTrue(discovery.contains("let wireData = try Self.primaryBonjourInteropAdvertisementWireData("))
        XCTAssertTrue(discovery.contains("return NWTXTRecord(wireData)"))
        XCTAssertFalse(discovery.contains("for (key, value) in fields"))
        XCTAssertFalse(discovery.contains("record[key] = value"))
        XCTAssertFalse(discovery.contains("record[\"uuid\"]"))
        XCTAssertFalse(discovery.contains("record[\"uniqueId\"]"))
        XCTAssertFalse(discovery.contains("record[\"protocolIdentityFingerprint\"]"))
        XCTAssertFalse(discovery.contains("record[\"vendorDeviceId\"]"))
        XCTAssertFalse(discovery.contains("record[\"capabilities\"]"))
        XCTAssertFalse(discovery.contains("record[\"port\"]"))
