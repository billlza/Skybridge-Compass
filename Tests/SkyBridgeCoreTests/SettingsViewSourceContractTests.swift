...
final class SettingsViewSourceContractTests: XCTestCase {
    ...
    func testRemoteDesktopSettingsAreBoundToRuntimeConsumers() throws {
        ...
        let nearFieldCanvas = try repositorySource("Sources/SkyBridgeCompassApp/Views/NearFieldControlCanvas.swift")
        ...
        XCTAssertTrue(nearFieldView.contains("NearFieldControlCanvas("))
        XCTAssertTrue(nearFieldCanvas.contains("RemoteDesktopSettingsManager.shared.settings.interactionSettings"))
        XCTAssertTrue(nearFieldCanvas.contains("guard settings.enableContextMenu else { return }"))
