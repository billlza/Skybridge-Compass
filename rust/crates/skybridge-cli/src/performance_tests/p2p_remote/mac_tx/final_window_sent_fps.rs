...
fn p2p_remote_mac_final_window_rejects_low_min_sent_fps_inside_final_window() -> Result<()> {
    ...
    for second in 1..=8 {
        let sent = if second == 2 { 27 } else { 60 };
        ...
        mac_log.push_str(&format!(
            "[2026-05-15T00:00:{second:02}Z] mac-remote-frame-tx transport=sbc2-chunked-v1 sampleMs=1000 sent={sent} wireBatchSingleFrames={sent} wireBatchMultiFrames=0 wireSingleUnbatchedFrames=0 chunkedFrames={sent} sentChunks={sent} maxChunksPerFrame=1 sentFPS={sent_fps:.1} maxSendMs=12.0 writerClock=dispatch-source-userinteractive sendScheduler=dispatch-clock-only maxFramesPerDrain=1 scheduleBudgetMax=1 missedCadenceSlotsMax=0 dropped=0 backpressure=0 rawBackpressure=0 contentBacklogFull=0 contentBacklogMax=1 contentBacklogLimit=18 contentBacklogBytesMax=8192 contentBacklogByteLimit=3145728 staleQueueCatchUp=0 queuedMax=1 queueBacklog=0 queueAgeMaxMs=12.0 waitingForSync=false\n"
        ));
    }
    ...
    assert!(check.detail.contains("finalWindowSentFPS=55."));
...
#[test]
fn p2p_remote_mac_final_window_reports_transient_source_frame_age_as_diagnostic() -> Result<()> {
    let artifact_dir = make_test_dir("p2p-remote-final-window-source-age-diagnostic")?;
    std::fs::write(
        artifact_dir.join("ios-p2p-remote-TEST.status.log"),
        "[2026-05-15T00:00:00Z] remote-desktop pass-window-start windowSeconds=10 windowFPS=60.0 windowRxFps=60.0 frame=2056x1329\n\
         [2026-05-15T00:00:10Z] remote-desktop-pass seconds=10 requestedSeconds=10 windowSeconds=10 windowFPS=60.0 windowRxFps=60.0 windowDisplayedFrames=600 windowReceivedFrames=600 twoSecondRequiredFrames=118 min2sDisplayFrames=118 min2sRxFrames=118 rollingCadencePass=1 frame=2056x1329\n",
    ...
    assert!(check.ok, "{}", check.detail);
    assert!(check.detail.contains("sckSourceFrameAgeMaxMs=Some(38.6)"));
    assert!(
        check
            .detail
