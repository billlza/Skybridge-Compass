...
fn p2p_remote_final_window_accepts_ios_local_console_timestamps() -> Result<()> {
    ...
    for second in 1..=P2P_REMOTE_MIN_METAL_TELEMETRY_SAMPLES {
        ...
        ios_log.push_str(&format!(
            "{stamp} ios-lan-remote-rx sampleMs=1000 screenFrames=60 screenFPS=60.0 maxGapMs=16.7 sourceSamples=60 sourceGapMaxMs=16.7 sourceToReadMaxMs=20.0 sourceToReadClock=remote-wall-clock-unsynced rawChunks=60 rawChunkGapMaxMs=18.0 rawChunkMainHopMaxMs=1.0 maxMainHopMs=1.0 completeFramesPerDrainMax=1 parserDrainMaxMs=1.0 parserBudgetMs=6.0 parserBudgetHits=0 parser=secure-off-main-actor parseQueueDelayMaxMs=1.0 parserActorHopMaxMs=1.0 parserStageMax=decode parserStageMaxMs=1.0 parserStagePayloadBytesMax=65536 parserStageBufferBytesMax=65536 applyQueueDelayMaxMs=1.0 screenApplyMaxMs=1.0 screenWire=sbc2-chunked-v1 readAhead={P2P_REMOTE_STRICT_IOS_READ_AHEAD_MODE} rxFrameClock=socket-arrival screenDelivery={P2P_REMOTE_STRICT_IOS_SCREEN_DELIVERY_MODE} screenDeliveryAttempted=60 screenDeliveryDelivered=60 screenDeliveryBackpressure=0 screenDeliveryQueueDepthMax=1 screenDeliveryDelayMaxMs=10.0 decodeFeed={P2P_REMOTE_STRICT_IOS_DECODE_FEED_MODE} socketMetricClock=local-socket-arrival socketToDecodeFeedSamples=60 socketToDecodeFeedMaxMs=2.0 socketToApplyEndSamples=60 socketToApplyEndMaxMs=3.0 decodeAttempted=60 decodeAccepted=60 decodeDropped=0 decodePendingMax=1 decodeInFlightMax=1 decodeWaitingSyncSamples=0 decodeResets=0 sbc2Frames=60 sbc2Chunks=60\n"
        ));
        ios_log.push_str(&format!(
