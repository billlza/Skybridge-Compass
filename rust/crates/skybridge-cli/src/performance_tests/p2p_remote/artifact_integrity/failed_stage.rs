...
#[test]
fn p2p_remote_artifact_rejects_already_connected_rejection() -> Result<()> {
    let artifact_dir = make_test_dir("p2p-remote-already-connected")?;
    std::fs::write(
        artifact_dir.join("mac.status.log"),
        "smoke-capture-source captureVerified=1 changedRatio=0.500 meanDelta=12.0\nmac-stream-config codec=hevc fps=60 fallback=fail-fast\nsmoke-final result=success validated=1 route=lan-direct fps=59 frame=2056x1329\n",
    )?;
    ...
    assert!(
        hidden_failure
            .detail
            .contains("alreadyConnectedRejections=1")
    );
    assert_eq!(report.fault_stage, Some("p2p_remote_already_connected"));
    Ok(())
}

#[test]
fn p2p_remote_artifact_rejects_plain_remote_desktop_connection_failure() -> Result<()> {
    let artifact_dir = make_test_dir("p2p-remote-plain-remote-desktop-failure")?;
    std::fs::write(
        artifact_dir.join("mac.status.log"),
        "smoke-capture-source captureVerified=1 changedRatio=0.500 meanDelta=12.0\nmac-stream-config codec=hevc fps=60 fallback=fail-fast\nsmoke-final result=success validated=1 route=lan-direct fps=59 frame=2056x1329\n",
    )?;
    std::fs::write(
        artifact_dir.join("ios-p2p-remote-TEST.status.log"),
        "remote-desktop-pass fps=60.0 rxFps=60.0 frame=2056x1329\nsmoke-final result=success validated=1 route=lan-direct fps=59 frame=2056x1329\n[17:17:23.823] [ERROR] [General] ❌ 远程桌面连接失败: 缺少真实 SkyBridge 控制端点\n",
    )?;
    let args = PerformanceCheckArgs {
        kind: PerformanceKindArg::P2pRemote,
        session_id: None,
