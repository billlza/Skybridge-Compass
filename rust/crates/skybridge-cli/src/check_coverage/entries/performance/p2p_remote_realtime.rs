...
pub(super) fn append_entries(
    ...
    entries.push(CheckCoverageEntry {
        ...
        evidence: "requires iOS display/rx plus Mac SCK and sender FPS evidence inside a sustained final pass window, preventing short-window or late-sample fake passes".to_owned(),
    });
    entries.push(CheckCoverageEntry {
        ...
        evidence: "requires raw NWConnection receive gaps, MainActor handoff, bounded complete-frame parser drains, and strict decoded-to-Metal 60Hz delivery to stay inside the 2K60 receive budget".to_owned(),
    });
}
