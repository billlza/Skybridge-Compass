    ...
    I: Interceptor,
{
    ...
    pub(super) fn update_ice_agent_stats(&mut self) {
            ...
            .get_selected_candidate_pair()
        {
            ...
            // LocalCandidate/RemoteCandidate stats entry. `get_stats` consumers
            // that resolve the selected pair's endpoints (W3C-conformant
            // lookups) then never observe the route. Register the selected
            // pair's candidates here — the one place the full candidates are
            // in hand — before the snapshot is taken. Keys mirror the
            // registration paths above; existing entries are not overwritten.
            let local_key = format!("RTCLocalIceCandidate_{}", local.id());
            if !self.pipeline_context.stats.has_local_candidate(&local_key) {
                let rtc_local: RTCIceCandidate = local.into();
                let (ufrag, _) = self.ice_transport().get_local_user_credentials();
                let accumulator = self.candidate_to_accumulator(&rtc_local, ufrag, None);
                self.pipeline_context
                    .stats
                    .register_local_candidate(local_key, accumulator);
            }
            let remote_key = format!("RTCRemoteIceCandidate_{}", remote.id());
            if !self.pipeline_context.stats.has_remote_candidate(&remote_key) {
                let rtc_remote: RTCIceCandidate = remote.into();
                let (ufrag, _) = self.ice_transport().get_remote_user_credentials();
                let accumulator = self.candidate_to_accumulator(&rtc_remote, ufrag, None);
                self.pipeline_context
                    .stats
                    .register_remote_candidate(remote_key, accumulator);
            }
