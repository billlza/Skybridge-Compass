...
impl RTCStatsAccumulator {
    ...
    /// SKYBRIDGE PATCH: presence probes so the selected-pair registration in
    /// `update_ice_agent_stats` never clobbers richer entries recorded by the
    /// normal registration paths.
    pub(crate) fn has_local_candidate(&self, id: &str) -> bool {
        self.local_candidates.contains_key(id)
    }

    pub(crate) fn has_remote_candidate(&self, id: &str) -> bool {
        self.remote_candidates.contains_key(id)
    }
