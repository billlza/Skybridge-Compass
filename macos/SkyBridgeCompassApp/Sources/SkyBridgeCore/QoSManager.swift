...
public final class QoSManager: @unchecked Sendable {
    
    // MARK: - 单例模式
    public static let shared = QoSManager()
    
    // MARK: - 私有属性
    private let logger = Logger(subsystem: "com.skybridge.compass", category: "QoSManager")
    ...
    public func recommendedConcurrency(for taskType: TaskType) async -> Int {
        ...
        if #available(macOS 14.0, *), let optimizer = optimizer {
            ...
            systemInfo = AppleSiliconOptimizer.SystemInfo(
                ...
                performanceCoreCount: ProcessInfo.processInfo.processorCount / 2,
                efficiencyCoreCount: ProcessInfo.processInfo.processorCount / 2,
                cacheLineSize: 64,
                pageSize: 4096
            )
        }
