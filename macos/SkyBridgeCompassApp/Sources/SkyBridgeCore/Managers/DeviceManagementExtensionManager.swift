        ...
        await withTaskGroup(of: Void.self) { group in
            ...
            group.addTask {
                do {
                    ...
                    await MainActor.run {
                        self.registeredExtensions[extensionId] = `extension`
    ...
    public func scanAllDevices() async throws -> [any DeviceRepresentable] {
        ...
        for scanner in enabledScanners {
            do {
                ...
                print("⚠️ 扫描器 \(scanner.extensionName) 扫描失败: \(error.localizedDescription)")
            }
        }
    ...
    public func connectDevice(_ device: any DeviceRepresentable, preferredProtocol: String? = nil) async throws -> Bool {
            ...
            for connector in enabledConnectors {
                if connector.supportedProtocols.contains(preferredProtocol) {
                    do {
                        ...
                        print("⚠️ 使用首选协议 \(preferredProtocol) 连接失败: \(error.localizedDescription)")
                    }
                }
            }
        }
        ...
        for connector in enabledConnectors {
            do {
                ...
                print("⚠️ 连接器 \(connector.extensionName) 连接失败: \(error.localizedDescription)")
                continue
            }
        }
    ...
    public func processDeviceData(_ data: Data, from device: any DeviceRepresentable, dataType: String) async throws -> ProcessedDeviceData? {
        ...
        for processor in processors {
            do {
                ...
                print("⚠️ 数据处理器 \(processor.extensionName) 处理失败: \(error.localizedDescription)")
            }
        }
...
private struct ExtensionRowView: View {
    ...
    var body: some View {
        HStack {
            ...
            // 启用状态切换
            Toggle("", isOn: .constant(extensionItem.isEnabled))
    ...
    private var statusColor: Color {
        ...
        case .loading:
            return .orange
        case .loaded:
            return extensionItem.isEnabled ? .green : .gray
        case .failed:
            return .red
        }
    }
