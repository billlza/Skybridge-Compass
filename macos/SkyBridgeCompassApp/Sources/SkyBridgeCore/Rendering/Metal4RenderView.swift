...
public struct Metal4RenderView: NSViewRepresentable {
    ...
    public func makeNSView(context: Context) -> MTKView {
        ...
        // 创建渲染器
        do {
            let renderer = try Metal4Renderer(
                metalView: metalView,
    ...
    public class Coordinator: NSObject, MTKViewDelegate {
        var renderer: Metal4Renderer? // 修正为正确的类型
...
class Metal4Renderer: NSObject, MTKViewDelegate {
    ...
    func draw(in view: MTKView) {
        ...
        Task {
            do {
                try await renderingEngine.renderWeatherScene(
    ...
    private var advancedSettingsView: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(alignment: .leading) {
                ...
                Toggle("显示线框", isOn: .constant(false))
                Toggle("显示法线", isOn: .constant(false))
                Toggle("显示深度缓冲", isOn: .constant(false))
            }
        }
        .padding()
