...
// MARK: - 设备信息
public struct P2PDeviceInfo: Codable, Identifiable {
...
public struct P2PDevice: Codable, Identifiable, Hashable, Sendable {
    ...
    public init(from deviceInfo: P2PDeviceInfo) {
        ...
        self.publicKey = Data() // 临时空数据，实际应从设备信息中获取
        self.lastSeen = Date()
        self.endpoints = ["\(deviceInfo.address):\(deviceInfo.port)"] // 基于地址和端口创建端点
    }

    public init(id: String, name: String, type: P2PDeviceType, address: String, port: UInt16, osVersion: String, capabilities: [String], publicKey: Data, lastSeen: Date, endpoints: [String] = []) {
        self.id = id
        self.name = name
        self.type = type
