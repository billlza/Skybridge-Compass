...
public final class AppleSiliconFeatureDetector: @unchecked Sendable {
    ...
    private func detectSystemInfo() -> SystemInfo {
        ...
        if isAppleSilicon {
            ...
            if result == 0 {
                // 获取芯片型号信息，使用推荐的方法
                let bufferArray = Array(UnsafeBufferPointer(start: buffer, count: size))
        ...
        if isAppleSilicon {
            ...
            // Intel Mac，所有核心视为性能核心
            var totalCores: UInt64 = 0
            size = MemoryLayout<UInt64>.size
            result = sysctlbyname("hw.logicalcpu", &totalCores, &size, nil, 0)
            if result == 0 {
                performanceCoreCount = Int(totalCores)
            }
        }
        
        // 检测特殊功能支持
        let supportsUnifiedMemory = isAppleSilicon
        let supportsNeuralEngine = isAppleSilicon && (chipModel.contains("M1") || chipModel.contains("M2") || chipModel.contains("M3"))
        let supportsAMX = isAppleSilicon
        
        // 估算热设计功耗
        var thermalDesignPower: Double = 10.0 // 默认值
        if isAppleSilicon {
            if chipModel.contains("Pro") || chipModel.contains("Max") {
                ...
                thermalDesignPower = 15.0
            }
        } else {
            thermalDesignPower = 45.0 // Intel Mac估算值
        }
