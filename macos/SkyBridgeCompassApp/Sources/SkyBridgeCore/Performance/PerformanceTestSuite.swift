...
public final class PerformanceTestSuite: @unchecked Sendable {
    ...
    private func testMemoryOptimization() async -> TestResult {
        ...
        let allocator = memoryOptimizer.getOptimizedAllocator(for: Double.self, count: largeDataSize)
    ...
    private func testVectorization() async -> TestResult {
        ...
        // 使用向量化操作 - 使用平方运算
        let vectorizedResult = optimizer.performVectorizedOperation(testData.map(Float.init), operation: .square)
    ...
    private func testImageProcessing() async -> TestResult {
        ...
        // 使用向量化操作进行图像滤波
        let blurredImage = optimizer.performVectorizedOperation(imageData, operation: .sqrt)
    ...
    private func testDataAnalysis() async -> TestResult {
        ...
        // 计算方差
        var meanArray = [Double](repeating: mean, count: dataSize)
