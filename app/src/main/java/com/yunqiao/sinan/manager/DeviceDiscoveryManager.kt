        ...
        discoveryJob = CoroutineScope(Dispatchers.IO).launch {
            while (isDiscovering) {
                try {
                    ...
                    val discoveryPreferences = ConfigurationManager.configurationSnapshot().discovery
                    val interval = max(discoveryPreferences.wifiScanIntervalMillis, 2000L)
                    delay(interval)
    ...
    private fun discoverBLEDevices() {
        ...
        if (ActivityCompat.checkSelfPermission(
            ...
            try {
                ...
                isBleScanning = true
                val duration = max(ConfigurationManager.configurationSnapshot().discovery.bleScanDurationMillis, 5000L)
                bleScanTimeoutJob?.cancel()
                bleScanTimeoutJob = CoroutineScope(Dispatchers.IO).launch {
                    delay(duration)
                    stopBleScan()
                }
