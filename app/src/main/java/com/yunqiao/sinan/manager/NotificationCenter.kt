...
class NotificationCenter {
    ...
    fun onDeviceStatusChanged(status: DeviceStatus) {
        if (lastConnectionStatus != status.connectionStatus) {
            ...
            if (status.connectionStatus == ConnectionStatus.ONLINE && now - lastThroughputTimestamp >= 5000) {
                ...
                _notifications.value = listOf(throughputNotification) + _notifications.value.take(9)
