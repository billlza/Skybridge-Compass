...
class RemoteDesktopManager(private val context: Context) {
    ...
    private val config = Android16PlatformBoost.tuneConfig(
        ConfigurationManager.configurationSnapshot().remoteDesktop.let { preferences ->
            RemoteDesktopConfig(
                maxSessions = max(preferences.maxSessions, 1),
                targetFps = when (preferences.qualityProfile.lowercase(Locale.ROOT)) {
                    "enterprise" -> 120,
                    "lossless" -> 144,
                    "pro" -> 120,
                    else -> 60
                },
                targetBitrate = when (preferences.qualityProfile.lowercase(Locale.ROOT)) {
                    "enterprise" -> 9000,
                    "lossless" -> 12000,
                    "pro" -> 10000,
                    else -> 5000
                },
                enableHardwareAcceleration = preferences.hardwareAcceleration
            )
        }
    )
    ...
    private val _projectionRequests = MutableSharedFlow<Unit>(extraBufferCapacity = 1, onBufferOverflow = BufferOverflow.DROP_OLDEST)
    val projectionRequests: SharedFlow<Unit> = _projectionRequests.asSharedFlow()
    ...
    private sealed interface PendingConnection {
        data class Device(val deviceId: String, val fallbackAccount: String?) : PendingConnection
        data class Account(val accountId: String) : PendingConnection
    }
    ...
    private suspend fun handleClientConnection(clientSocket: Socket) = withContext(Dispatchers.IO) {
        ...
        try {
            val capacity = max(ConfigurationManager.configurationSnapshot().remoteDesktop.maxSessions, 1)
            if (sessionMap.size >= capacity) {
    ...
    fun setMediaProjection(projection: MediaProjection) {
        ...
        if (pending != null) {
            ...
            scope.launch {
                when (pending) {
                    is PendingConnection.Device -> connectToDeviceInternal(pending.deviceId, pending.fallbackAccount)
                    is PendingConnection.Account -> connectViaAccountInternal(pending.accountId)
                }
            }
        }
    }
    ...
    fun connectToDevice(deviceId: String, fallbackAccount: String? = null) {
        scope.launch {
            if (!ensureProjectionReady(PendingConnection.Device(deviceId, fallbackAccount))) {
                return@launch
            }
            connectToDeviceInternal(deviceId, fallbackAccount)
        }
    }

    fun connectViaAccount(accountId: String) {
        scope.launch {
            if (!ensureProjectionReady(PendingConnection.Account(accountId))) {
                return@launch
            }
            connectViaAccountInternal(accountId)
        }
    }

    private fun ensureProjectionReady(pending: PendingConnection): Boolean {
        if (mediaProjection != null) {
            return true
        }
        pendingConnection = pending
        _connectionStatus.value = "awaiting_projection"
        _projectionRequests.tryEmit(Unit)
        return false
    }

    private suspend fun connectToDeviceInternal(deviceId: String, fallbackAccount: String?) {
        try {
            _connectionStatus.value = "connecting"
            val transport = bridgeCoordinator.negotiateTransport(deviceId, fallbackAccount)
            val connected = startTransport(transport)
    ...
    private suspend fun connectViaAccountInternal(accountId: String) {
        try {
            _connectionStatus.value = "connecting"
    ...
    private suspend fun startTransport(transport: BridgeTransport): Boolean {
