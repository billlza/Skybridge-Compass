...
/// - Default state: Background is Sleeping (Paused/Hidden) to save resources.
...
public class BackgroundControlManager: ObservableObject {
 ...
 // Weather ON: Start logic
 // If we just switched weather on, start idle timer
            startIdleTimer()
        }
    }
 ...
 // Reset Idle Timer
    ...
    private func handleIdleTimeout() {
        if isWeatherActive {
 // Weather ON: Start Fade Out logic
            startFadeOut()
        } else {
 // Weather OFF: Start FPS Throttling logic
            startThrottling()
        }
    }
