...
public enum P2PCryptoAlgorithm: String, Codable, Sendable {
 // KEM 算法
    ...
    /// Legacy ABI 1 identifier. Decode-only; never advertise for a new session.
    case qperiaptContextBound = "Q-Periapt-ContextBound"
    /// ABI 2 authenticated-policy/application-context-bound hybrid KEM.
    case qperiaptABI2PolicyBound = "Q-Periapt-ABI2-PolicyBound"
 ...
 /// 是否为 PQC 算法
    public var isPQC: Bool {
        switch self {
        case .xWing, .mlKEM768, .mlDSA65, .qperiaptContextBound, .qperiaptABI2PolicyBound:
            return true
        default:
            return false
        }
