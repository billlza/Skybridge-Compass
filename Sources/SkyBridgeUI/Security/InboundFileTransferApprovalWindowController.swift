...
/// AppKit presenter for the same explicit file-receive approval surface used by the
/// shipping SwiftUI application.
///
/// There is intentionally no automatic decision path. Closing the window without
/// a decision rejects the request through `InboundFileTransferApprovalService`; service
/// cancellation and timeout remain fail closed as well.
@available(macOS 14.0, *)
@MainActor
public final class InboundFileTransferApprovalWindowController: NSObject, NSWindowDelegate {
    private let approvalService: InboundFileTransferApprovalService
    ...
    init(
        approvalService: InboundFileTransferApprovalService,
    ...
    private func synchronizeWindow(with publishedRequest: InboundFileTransferApprovalService.Request?) {
    ...
    private func presentApprovalWindow(for request: InboundFileTransferApprovalService.Request) {
        let content = InboundFileTransferApprovalSheet(
            request: request,
            onDecision: { [weak self] decision in
                guard let self,
                      self.approvalService.pendingRequest?.id == request.id else {
                    return
                }
        ...
        window.title = "SkyBridge File Transfer Approval"
