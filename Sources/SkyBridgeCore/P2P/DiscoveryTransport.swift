...
public actor BonjourDiscoveryTransport: DiscoveryTransport {
 ...
 /// 开始接收数据
    private func startReceiving(on connection: NWConnection, from peer: PeerIdentifier) {
        receiveNextMessage(on: connection, from: peer)
    }
    
 /// 接收下一条消息
    private func receiveNextMessage(on connection: NWConnection, from peer: PeerIdentifier) {
 // 先读取 4 字节长度前缀
 ...
 // 继续接收
                Task {
                    await self.receiveNextMessage(on: connection, from: peer)
                }
                return
            }
