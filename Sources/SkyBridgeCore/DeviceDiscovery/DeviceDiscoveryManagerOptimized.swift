...
public class DeviceDiscoveryManagerOptimized: ObservableObject {
    ...
    private final class BonjourServiceResolveContext: NSObject, NetServiceDelegate, @unchecked Sendable {
        ...
        func netServiceDidResolveAddress(_ sender: NetService) {
            ...
            finish(.success(ResolvedBonjourService(
                ...
                ipv4: foundIPv4,
                ipv6: foundIPv6,
                txt: DeviceDiscoveryManagerOptimized.parseNetServiceTXT(sender.txtRecordData())
            )))
        }
 ...
 /// 刷新待处理的更新（批量UI更新）
    private func flushPendingUpdates() async {
        ...
        do {
            ...
            logger.error(
                "❌ Discovery batch blocked because local identity authority is unavailable: \(SkyBridgeDiagnosticRedaction.errorSummary(error), privacy: .public)"
            )
            return
        }
 ...
 // 因此这里仅返回端口，地址留空，连接时优先走 Bonjour service endpoint。
    ...
    private func publishHydratedPrimaryControlService(
        ...
        let txt = resolved.txt
        let identity = Self.protocolIdentity(fromTXT: txt)
        let metadata = Self.deviceInfo(fromTXT: txt) ?? fallbackMetadata
        let controlPort = P2PDiscoveryBonjourPolicy.advertisedServicePort(
            from: txt,
    ...
    private nonisolated static func protocolIdentity(fromTXT txt: [String: String]) -> (deviceId: String?, pubKeyFP: String?) {
        let deviceId = sanitizeStableDeviceId(
            txtValue(txt, keys: ["deviceId", "id", "deviceID", "device_id", "uuid", "uniqueId", "unique_id"])
        )
        let pubKeyFP = sanitizePubKeyFingerprint(
            txtValue(txt, keys: ["pubKeyFP", "pubKeyFp", "pubkeyfp", "pub_key_fp", "identityFingerprint"])
        )
        return (deviceId, pubKeyFP)
    }

    private nonisolated static func deviceInfo(fromTXT txt: [String: String]) -> BonjourDeviceInfo? {
        let info = BonjourDeviceInfo(
            deviceId: protocolIdentity(fromTXT: txt).deviceId,
            hostname: txtValue(txt, keys: ["hostname", "host"]),
            model: txtValue(txt, keys: ["model", "modelName", "md"]),
            chip: txtValue(txt, keys: ["chip", "soc", "cpu"]),
            type: txtValue(txt, keys: ["type", "deviceType"]),
            version: txtValue(txt, keys: ["version", "ver", "sw"]),
            osVersion: txtValue(txt, keys: ["osVersion", "os_version", "osver", "osVer", "osv"]),
            manufacturer: txtValue(txt, keys: ["manufacturer", "brand", "mf"]),
            platform: txtValue(txt, keys: ["platform", "os"]),
            name: txtValue(txt, keys: ["name", "device", "fn"]),
            remoteVideoFormats: BonjourInteropContract.normalizedRemoteVideoFormats(
                from: txtValue(txt, keys: Array(BonjourInteropContract.remoteVideoFormatTXTKeys))
            )
        )
    ...
    private nonisolated static func txtValue(_ txt: [String: String], keys: [String]) -> String? {
        for key in keys {
            guard let value = txt[key]?.trimmingCharacters(in: .whitespacesAndNewlines),
                  !value.isEmpty else {
                continue
            }
            return value
    ...
    private nonisolated static func parseNetServiceTXT(_ data: Data?) -> [String: String] {
...
// 精确移除（基于设备名称）
 ...
 // 使用统一广播中心，确保同一服务类型只存在一个 NWListener，且运行在全局队列
            do {
                ...
                let model = try await SelfIdentityProvider.shared
                    .getRegistrationDeviceInfo(allowCreate: false)
                    .hardwareModel
                ...
                self.logger.error("❌ 启动广播服务失败: \(SkyBridgeDiagnosticRedaction.errorSummary(error), privacy: .public)")
            }
        }
    }
 ...
 // 门闩去重——若同类型广播已在运行则直接返回，避免 stop→start 风暴
            ...
            bgLogger.error("❌ 启动广播服务失败: \(SkyBridgeDiagnosticRedaction.errorSummary(error), privacy: .public)")
        }
    }
 ...
 // MARK: - Inbound control channel (HandshakeDriver compatibility)
    ...
    nonisolated private static func consumeInboundHandshakeOrControlChannel(_ connection: NWConnection) async {
        ...
        var previousSessionKeysBeforeRekey: SessionKeys? = nil
        var previousAuthenticatedRemoteAuthorityBeforeRekey: AuthenticatedRemoteAuthority? = nil
        ...
        let endpointDiagnosticLabel = SkyBridgeDiagnosticRedaction.stableIdentifierLabel(
            ...
            while true {
                ...
                if let messageA = try? HandshakeMessageA.decode(from: frame),
                   !messageA.hasNegotiableOfferShape {
                    logger.error(
                        "⛔️ Rejected non-negotiable inbound MessageA before rekey state mutation. peer=\(SkyBridgeDiagnosticRedaction.stableIdentifierLabel(peerIdForPresence), privacy: .public)"
                    )
                    continue
                }
                ...
                if let currentDriver = driver,
                   let messageA = try? HandshakeMessageA.decode(from: frame) {
                    ...
                    case .waitingFinished, .established:
                        ...
                        logger.info("🔁 入站 transactional rekey candidate：\(fromKind)·\(fromSuite) -> \(toKind)·\(toSuite) peer=\(SkyBridgeDiagnosticRedaction.stableIdentifierLabel(peerIdForPresence), privacy: .public)")
                        ...
                        if let msg = try? JSONDecoder().decode(AppMessage.self, from: plaintext) {
                            ...
                            case .pairingIdentityExchange(let payload):
                                ...
                                guard isCurrentPairingOperation(
                                    ...
                                    _ = await rollbackPairingCommit(
                                        pairingReservation,
                                        reason: "stale_after_reply_prepare"
                                    )
                                    return
                                }
                                ...
                                if let replyMaterial {
                                    do {
                                        try await transport.send(to: peer, data: replyMaterial.payload)
                                    } catch {
                                        logger.error(
                ...
                if let activeDriver = driver {
                    ...
                    case .failed(let reason):
                        if let previousKeys = previousSessionKeysBeforeRekey {
                            ...
                            logger.warning(
                                "⚠️ inbound rekey failed; restored previous authenticated session. peer=\(SkyBridgeDiagnosticRedaction.stableIdentifierLabel(peerIdForPresence), privacy: .public) reason=\(reason.diagnosticReasonCode, privacy: .public) suite=\(previousKeys.negotiatedSuite.rawValue, privacy: .public)"
                            )
                        }
                    default:
                        break
