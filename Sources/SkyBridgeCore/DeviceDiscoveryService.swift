...
public final class DeviceDiscoveryService: ObservableObject {
 ...
 // 应用即将进入活跃状态
        NotificationCenter.default.addObserver(
            ...
            Task { @MainActor in
                ...
                SkyBridgeLogger.discovery.debugOnly("🔄 应用恢复活跃，重启设备发现...")
                await self.start()
            }
        }
 ...
 // 应用即将失去活跃状态
        NotificationCenter.default.addObserver(
            ...
            Task { @MainActor in
                guard let self = self else { return }
                SkyBridgeLogger.discovery.debugOnly("⏸ 应用失去活跃，暂停设备发现...")
                self.stop()
        ...
        Task { @MainActor in
            await withTaskGroup(of: Void.self) { group in
                let scanTasks = [
                     ...
                     ("端口扫描", TaskType.dataAnalysis),
                     ("Bonjour扫描", TaskType.networkRequest)
                 ]
 ...
 /// 为指定设备生成候选稳定指纹（不阻塞UI）。
    private func makeFingerprint(for device: DiscoveredDevice) async -> IdentityFingerprint? {
        let ip4 = device.ipv4
        let ip6 = device.ipv6
        let mac = ip4.flatMap { arpCacheIPv4[$0] } ?? ip6.flatMap { ndpCacheIPv6[$0] }
        ...
        if let cached = await IdentityResolver.WeakFingerprintStore.shared.load(for: device) {
