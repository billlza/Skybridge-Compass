...
- `Scripts/verify-windows-ui-automation-smoke.ps1 -EvidenceDir <dir>` captures the rendered WinUI shell for all eight workspaces at requested logical window sizes 1280x900 and 1366x768 and writes `windows-ui-visual-evidence.json` with requested size, actual screenshot pixel size, selected feature id, the corresponding same-viewport automation-anchor bounds, and `runtimeActionBounds` generated from this matrix's Action Order Matrix. Device Discovery's Local Scan, QR, and Code action surfaces are verified by switching the real mode tabs and requiring the first expected action id for that mode before bounds are accepted. Scrollable surfaces such as Settings maintenance actions are verified by moving the real workspace ScrollViewer to the required surface, then returning to the top before screenshot capture. The runtime bounds gate verifies every visible global and selected-workspace `WorkspaceAction.<Surface>.<Key>` AutomationId exists with minimum usable bounds and flows in matrix order before the screenshot evidence is accepted; global session controls are covered there instead of being required as screenshot anchors in every workspace frame. These artifacts are the local visual evidence package for mac/iOS comparison while fonts, OS scale, and platform pixel metrics remain out of scope.
- `Scripts/verify-windows-ui-visual-evidence.ps1 -EvidenceDir <dir>` is the standalone artifact verifier for that package. It checks the manifest, 16 PNG screenshots, all eight workspaces, selected feature ids, both requested window sizes, anchor bounds, screenshot headers, and every recorded `runtimeActionBounds` surface/key/order against this Action Order Matrix so visual evidence can be revalidated after capture.
