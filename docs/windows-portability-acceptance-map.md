...
| `REQ-BASIC-SMOKE` CLI/basic operations and repository smoke paths are executable | `Scripts/verify-windows-portability-smoke.ps1` runs default static/service/CLI proof gates, and CI runs it with `-CiMode -CheckOnlineStackFreshness -IncludeRustCliCoverage`. |
