...
- do not retain authenticated TURN credential artifacts. `Scripts\verify-windows-mac-webrtc-helper-live.ps1` rejects `-KeepArtifacts` when `-IceServerCredentialsPath` is supplied, copies the credential JSON only into the per-run Mac helper temp directory, applies private permissions there, and fails closed if cleanup cannot remove the remote run directory after credentials were copied.
- prove the live helper generated the proof in the current run. `Scripts\verify-windows-mac-webrtc-helper-live.ps1` records its offer-run start time, rejects any proof whose `capturedAtUnixMs` predates that run, and prints proof hash plus last-write evidence for review.
