...
- Wire a Windows DNS-SD adapter to `IDiscoveryClient` once network discovery is introduced. The adapter should pass service names and TXT records to `CoreDiscoveryClient`, then hand the resulting `DiscoveredPeer.PublicKeyFingerprint` to the pairing/trust layer for verification instead of treating the fingerprint as a connectable public key.
...
- Windows FFI client tests currently use `Scripts/verify-windows-ffi-client.ps1` as a text-only gate for `IEngineClient` coverage, required C ABI imports, explicit state refresh, CoreBridge discovery parsing, CoreBridge frame codec coverage, the modular `IDiscoveryClient` boundary, and the no-silent-default-switch rule.
- FFI glue is covered in Rust integration tests for engine lifecycle, transport selection, channel mapping, frame codec roundtrip, and connection planning; Windows C# bindings should add text-only/interop tests once the WinUI test harness is introduced.
